...
void draw_turret(int x, int y, int d1, int d2, int angle, int angle_rad,
                 int angle_size, int col1, int col2, int col3);
...
void run_display(int show) {
  ...
  //  textprintf_right_ex(display, font, 595, 580, -1, -1, "%i fps",
  //  frames_per_second);
  /*  textprintf_ex(display, font, 10, 130, -1, -1, "%i slack", slacktime);
    textprintf_ex(display, font, 10, 150, -1, -1, "%i slack2",
    long_slacktime_store);*/
  /*
  ...
  */
...
#endif
  //  textprintf_ex(display, font, 10, 10, COL_WHITE, -1, "%i fps",
  //  frames_per_second);
  ...
  if (arena.level_finished > 0 && arena.level_finished < 390) {
    ...
    if (arena.level_finished < 360 && arena.level_finished > 20) {
      switch (arena.level) {
      case 1:
        textprintf_centre_ex(display, font, 300, 180, -1, -1,
                             "STAGE  ONE  COMPLETE");
        break;
      case 2:
        textprintf_centre_ex(display, font, 300, 180, -1, -1,
    ...
    //   textprintf_centre_ex(display, font, 320, 200, -1, -1, "SEE
    //   WWW.SHMUP-DEV.COM  FOR  THE  COMPETITION  VERSION!");
  }
...
void battle_report(void) {
  ...
  if (arena.level_finished < 310)
    textprintf_centre_ex(display, font, 300, 220, -1, -1,
                         "DESTRUCTION RATE: %i%%", arena.destruction_rate);
  ...
  if (arena.level_finished < 230) {
    ...
    case RANK_C:
      textprintf_ex(display, font, 310, 250, -1, -1, "C");
      break;
    case RANK_B:
      textprintf_ex(display, font, 310, 250, -1, -1, "B");
      break;
    case RANK_A:
      textprintf_ex(display, font, 310, 250, -1, -1, "A");
      textprintf_centre_ex(display, font, 300, 275, -1, -1, "WELL DONE");
      break;
    case RANK_S:
      textprintf_ex(display, font, 310, 250, -1, -1, "S");
      textprintf_centre_ex(display, font, 300, 275, -1, -1, "OUTSTANDING!");
      break;
    case RANK_P:
      textprintf_ex(display, font, 310, 250, -1, -1, "P");
      textprintf_centre_ex(display, font, 300, 275, -1, -1, "PERFECT!");
      break;
    }
  }
...
#define Y_PLACE 330
    ...
    textprintf_right_ex(display, font, 342, Y_PLACE - 2, -1, -1,
                        "base power level");

    TRANS_MODE

    for (k = 0; k < options.current_power; k++) {
      rectfill(display, 379 + k * 4, Y_PLACE + 3, 381 + k * 4, Y_PLACE + 8,
               TRANS_RED1);
      rect(display, 379 + k * 4, Y_PLACE + 3, 381 + k * 4, Y_PLACE + 8,
           TRANS_RED4);
    }
    ...
    if (options.current_wpixels > 0) {
      rectfill(display, 353, Y_PLACE + 3, 353 + options.current_wpixels,
               Y_PLACE + 8, TRANS_RED1);
      rect(display, 353, Y_PLACE + 3, 353 + options.current_wpixels,
           Y_PLACE + 8, TRANS_RED5);
    }
...
#define Y_SEP 30
    ...
    textprintf_right_ex(display, font, 342, Y_PLACE + Y_SEP - 2, -1, -1,
                        "previous best");

    TRANS_MODE

    for (k = 0; k < options.previous_power; k++) {
      rectfill(display, 379 + k * 4, Y_PLACE + 3 + Y_SEP, 381 + k * 4,
               Y_PLACE + 8 + Y_SEP, TRANS_RED1);
      rect(display, 379 + k * 4, Y_PLACE + 3 + Y_SEP, 381 + k * 4,
           Y_PLACE + 8 + Y_SEP, TRANS_RED4);
    }

    rect(display, 350, Y_PLACE + Y_SEP, 376, Y_PLACE + 11 + Y_SEP, TRANS_BLUE3);

    if (options.previous_wpixels > 0) {
      rectfill(display, 353, Y_PLACE + 3 + Y_SEP,
               353 + options.previous_wpixels, Y_PLACE + 8 + Y_SEP, TRANS_RED1);
      rect(display, 353, Y_PLACE + 3 + Y_SEP, 353 + options.previous_wpixels,
           Y_PLACE + 8 + Y_SEP, TRANS_RED5);
    }

    END_TRANS

    if ((options.current_power > options.previous_power) ||
        (options.current_power == options.previous_power &&
         options.current_bar > options.previous_bar))
      textprintf_centre_ex(display, font, 300, 420, -1, -1,
                           "new power level set");
  }
  ...
  if (arena.level_finished < 230 && arena.level == 3) {
    ...
    if (arena.cleared == 1)
      textprintf_centre_ex(display, font, 300, 380, -1, -1,
                           "full mission cleared!");
    if (arena.unlocked == 1)
      textprintf_centre_ex(display, font, 300, 420, -1, -1,
                           "chaos squid unlocked");
  }
}
...
int detect_player_collision(void) {
  ...
  //  textprintf_ex(display, font, 510, 100, -1, -1, "%i", getpixel(display, x,
  //  y)); textprintf_ex(display, font, 510, 100, -1, -1, "%i",
  //  player.hand_phase);

  if (check_pixel(x, y) || check_pixel(x + 1, y + 1) ||
  ...
  if ( // check_pixel(x, y) ||
      check_pixel(x - 2, y - 2) || check_pixel(x + 2, y - 2) ||
      check_pixel(x - 2, y + 2) || check_pixel(x + 2, y + 2) ||
...
void draw_player(int show) {
  ...
  case PTYPE_BASIC:
    ...
    xa = 4 + grand(4);
    draw_trans_rle_sprite(display, RLE_ccircle_basic[0][xa], x - xa - 1,
                          y - xa - 1 + 8);
    TRANS_MODE
    ya = 14 + grand(7);
    ...
    //   textprintf_right_ex(display, font, 300, 300, -1, -1, "%i, %i",
    //   player.x, player.y);
    break;
  case PTYPE_ANENOME:
    xa = S_PLAYER2_1;
    ...
    //    old_y2 = player.old_blade_y2 [j] / GRAIN;
        ...
        pline(display, old_x, old_y, player.old_blade_x [j] >> 10,
    player.old_blade_y [j] >> 10, TRANS_RED3);

    //     do_line(display, old_x, old_y, player.old_blade_x [j] >> 10,
    player.old_blade_y [j] >> 10, xa, draw_trail_line);
    ...
    //    xa *= 10 + xb;
    //    xa /= 10;
    //    bcircle(player.old_blade_x [j] >> 10, player.old_blade_y [j] >> 10,
    xa, 0);
  ...
  case PTYPE_BOMBER:
    xa = 4 + grand(4);
    ya = 0;
    draw_trans_rle_sprite(display, RLE_ccircle_basic[0][xa], x - xa - 1 - 16,
                          y - xa - 1 + 7);
    draw_trans_rle_sprite(display, RLE_ccircle_basic[0][xa], x - xa - 1 + 16,
                          y - xa - 1 + 7);
    xa = 3 + grand(3);
    ya += xa;
    draw_trans_rle_sprite(display, RLE_ccircle_4cols[0][xa], x - xa - 1 - 16,
                          y - xa - 1 + 7 + ya);
    draw_trans_rle_sprite(display, RLE_ccircle_4cols[0][xa], x - xa - 1 + 16,
                          y - xa - 1 + 7 + ya);
    xa = 2 + grand(3);
    ya += xa;
    draw_trans_rle_sprite(display, RLE_ccircle_3cols[0][xa], x - xa - 1 - 16,
                          y - xa - 1 + 7 + ya);
    draw_trans_rle_sprite(display, RLE_ccircle_3cols[0][xa], x - xa - 1 + 16,
                          y - xa - 1 + 7 + ya);
    ...
    bcircle(x + xpart(player.ring_angle, player.ring_size + 2),
            y + ypart(player.ring_angle, player.ring_size + 2), k, 0);
    k = 5 + player.weapon_level / 4 + grand(3);
    bcircle(x + xpart(player.ring_angle + ANGLE_3, player.ring_size + 2),
            y + ypart(player.ring_angle + ANGLE_3, player.ring_size + 2), k, 0);
    k = 5 + player.weapon_level / 4 + grand(3);
    bcircle(x + xpart(player.ring_angle - ANGLE_3, player.ring_size + 2),
            y + ypart(player.ring_angle - ANGLE_3, player.ring_size + 2), k, 0);
    ...
    */
    /*   xa = 4 + grand(4);
       draw_trans_rle_sprite(display, RLE_ccircle_basic [0] [xa], x - xa - 19, y
       - xa - 1 + Y_DISP); TRANS_MODE

       ya = 3 + grand(3);
       pline(display, x - 19, y + Y_DISP, x - 19 - ya, y + Y_DISP + ya,
       TRANS_RED4); yb = ya + 3 + grand(3); pline(display, x - 19 - ya, y +
       Y_DISP + ya, x - 19 - yb, y + Y_DISP + yb, TRANS_RED4); ya = yb + 3 +
       grand(3); pline(display, x - 19 - yb, y + Y_DISP + yb, x - 19 - ya, y +
       Y_DISP + ya, TRANS_RED3); yb = ya + 3 + grand(3); pline(display, x - 19 -
       ya, y + Y_DISP + ya, x - 19 - yb, y + Y_DISP + yb, TRANS_RED2); ya = yb +
       3 + grand(3); pline(display, x - 19 - yb, y + Y_DISP + yb, x - 19 - ya, y
       + Y_DISP + ya, TRANS_RED1);
    */
    //   END_TRANS_MODE
  ...
  case PTYPE_HAND:
    xa = 4 + grand(4);
    ya = 0;
    draw_trans_rle_sprite(display, RLE_ccircle_basic[0][xa],
                          x - xa - 1 - 15 - ya, y - xa - 1 + 3 + ya);
    draw_trans_rle_sprite(display, RLE_ccircle_basic[0][xa],
                          x - xa - 1 + 15 + ya, y - xa - 1 + 3 + ya);
    xa = 3 + grand(3);
    ya += xa / 2;
    draw_trans_rle_sprite(display, RLE_ccircle_4cols[0][xa],
                          x - xa - 1 - 15 - ya, y - xa - 1 + 5 + ya);
    draw_trans_rle_sprite(display, RLE_ccircle_4cols[0][xa],
                          x - xa - 1 + 15 + ya, y - xa - 1 + 5 + ya);
    xa = 2 + grand(3);
    ya += xa / 2;
    draw_trans_rle_sprite(display, RLE_ccircle_3cols[0][xa],
                          x - xa - 1 - 15 - ya, y - xa - 1 + 7 + ya);
    draw_trans_rle_sprite(display, RLE_ccircle_3cols[0][xa],
                          x - xa - 1 + 15 + ya, y - xa - 1 + 7 + ya);
  ...
  // draw_rle_sprite(display, eRLE_small1 [xa], x - 10, y - 7);
     draw_rle_sprite(display, eRLE_small1 [S_ENEMY_PLAYER_UP_L], x - 11 +
  player.wings1 / 10, y - 8); draw_rle_sprite(display, eRLE_small1
  [S_ENEMY_PLAYER_UP_R], x + 2 + player.wings1 / 10, y - 8);
     draw_rle_sprite(display, eRLE_small1 [S_ENEMY_PLAYER_DOWN_L], x - 10 -
  player.wings2 / 10, y + 2 - player.wings2 / 10); draw_rle_sprite(display,
  eRLE_small1 [S_ENEMY_PLAYER_DOWN_R], x + 2 + player.wings2 / 10, y + 2 -
  player.wings2 / 10);
  */
  //   xa = S_ENEMY_PLAYER1;
...
void draw_hand(void) {
  ...
  if (size > 0) {
    for (i = 0; i < 3; i++) {
      bcircle(x + xpart(player.hand_angle + ANGLE_3 * i,
                        (player.hand_unfold >> 1) + 2),
              y + ypart(player.hand_angle + ANGLE_3 * i,
                        (player.hand_unfold >> 1) + 2),
              size + grand(3), 2);
    }
  ...
  if (player.hand_unfold > 30 + player.weapon_level) {
    ...
    if (size > 0) {
      ...
      for (i = 0; i < 3; i++) {
        bcircle(x + xpart(angle + ANGLE_3 * i, (player.hand_unfold >> 2) + 3),
                y + ypart(angle + ANGLE_3 * i, (player.hand_unfold >> 2) + 3),
                size + grand(3), 3);
      }
...
void draw_HUD(void) {
  ...
  // textprintf_right_ex(display, font, 600, 43, -1, -1, "%i",
...
#define WPIXELS2 20
  ...
  if (player.weapon_level > 0) {
    ...
    if (player.overbflash > 0)
      textprintf_ex(display, font, 5, 43, -1, -1, "level %i!",
                    player.weapon_level);
...
#define BAR_X 8
    ...
    if (player.charge > 0) {
      TRANS_MODE
      if (player.charge >= 20000) {
        rectfill(display, BAR_X + 3, BAR_Y - 138, BAR_X + 18, BAR_Y - 2,
                 beam_colours[0][1 + (arena.counter / 4) % 4]);
      } else {
        ya = player.charge / 151;
        if (ya > 132)
          ya = 132;
        if (player.charge >= 6300)
          rectfill(display, BAR_X + 3, BAR_Y - 2 - ya, BAR_X + 18, BAR_Y - 2,
                   TRANS_RED4);
        else
          rectfill(display, BAR_X + 3, BAR_Y - 2 - ya, BAR_X + 18, BAR_Y - 2,
                   TRANS_RED2);
    ...
    /*    rect(display, BAR_X, BAR_Y - 51, BAR_X + 11, BAR_Y + 1, COL_OUTLINE);
        ...
        if (player.charge > 0)
        {
         ...
         if (player.charge >= 20000)
         {
          rectfill(display, BAR_X + 3, BAR_Y - 48, BAR_X + 8, BAR_Y - 2,
       beam_colours [0] [1 + (arena.counter / 4) % 4]);
         }
           else
           {
            ...
            if (ya > 46)
             ya = 46;
            rectfill(display, BAR_X + 3, BAR_Y - 2 - ya, BAR_X + 8, BAR_Y - 2,
       TRANS_RED3);
           }
         END_TRANS
        }
  ...
  case PTYPE_BOMBER:
    ...
    if (player.bomb_recycle1 <= 50) {
      rectfill(display, BAR_X + 3, BAR_Y - 138, BAR_X + 8, BAR_Y - 2,
               TRANS_RED5);
    } else {
    ...
    if (player.bomb_recycle2 <= 50) {
      rectfill(display, BAR_X + 12, BAR_Y - 138, BAR_X + 18, BAR_Y - 2,
               TRANS_RED5);
    } else {
      ...
      if (ya < 1)
        ya = 1;
      rectfill(display, BAR_X + 12, BAR_Y - 2 - ya, BAR_X + 18, BAR_Y - 2,
               TRANS_RED3);
    }
    END_TRANS
    break;
...
void draw_an_enemy(int e) {
  ...
  case ENEMY_BOSS2_1_DEAD:
    draw_rle_sprite(display, eRLE_large2[L_ENEMY2_BOSS2_2], x - 53,
                    y - 109 + (enemy[e].burst >> 10));
    ...
    xa = 18 + grand(3);
    ya = xa;
    bcircle(x, y - 45 + ya + (enemy[e].burst >> 10), xa + grand(6),
            CLOUD_COL_3);
    xa -= 3 + grand(4);
    ya += xa;
    bcircle(x, y - 45 + ya + (enemy[e].burst >> 10), xa + grand(6),
  ...
  case ENEMY_BOSS2_1:
    ...
    draw_rle_sprite(display, eRLE_large2[L_ENEMY2_BOSS2_1], x - 160, y - 109);
    draw_turret(x - 90, y + 27, 10, 13, enemy[e].aim[0], 8, 2, COL_COL4,
                COL_COL3, COL_COL2);
    draw_turret(x + 90, y + 27, 10, 13, enemy[e].aim[1], 8, 2, COL_COL4,
                COL_COL3, COL_COL2);
    draw_turret(x - 50, y - 13, 10, 13, enemy[e].aim[2], 8, 2, COL_COL4,
                COL_COL3, COL_COL2);
    draw_turret(x + 50, y - 13, 10, 13, enemy[e].aim[3], 8, 2, COL_COL4,
                COL_COL3, COL_COL2);
    //           draw_turret(x, y - 92, 7, 9, 0, 0, 0, COL_COL4, COL_COL3,
    //           COL_COL2);
  ...
  case ENEMY_BOSS1_3:
    ...
    draw_turret(x - 76, y - 11, 8, 12, enemy[e].aim[0], 6, 3, COL_COL4,
                COL_COL3, COL_COL2);
    draw_turret(x + 76, y - 11, 8, 12, enemy[e].aim[1], 6, 3, COL_COL4,
                COL_COL3, COL_COL2);
    draw_turret(x - 99, y + 42, 10, 14, enemy[e].aim[0], 0, 0, COL_COL4,
                COL_COL3, COL_COL2);
    draw_turret(x + 99, y + 42, 10, 14, enemy[e].aim[1], 0, 0, COL_COL4,
                COL_COL3, COL_COL2);
    ...
    //           draw_turret(x, y + 41, 10, 14, 0, 0, 0, COL_COL4, COL_COL3,
    //           COL_COL2);
    /*          xa = 8 + grand(5);
              ya = 0;
  ...
  case ENEMY_BOSS3_1:
    ...
    draw_turret(x - 55, y + 7, 10, 13, enemy[e].aim[0], 0, 0, COL_COL4,
                COL_COL3, COL_COL2);
    draw_turret(x - 106, y + 65, 9, 12, enemy[e].aim[0], 7, 2, COL_COL4,
                COL_COL3, COL_COL2);

    draw_turret(x + 55, y + 7, 10, 13, enemy[e].aim[0], 0, 0, COL_COL4,
                COL_COL3, COL_COL2);
    draw_turret(x + 106, y + 65, 9, 12, enemy[e].aim[1], 7, 2, COL_COL4,
                COL_COL3, COL_COL2);
  ...
  case ENEMY_BOSS3_2:
    ...
    draw_turret(x - 78, y - 8, 10, 13, enemy[e].aim[0], 8, 3, COL_COL4,
                COL_COL3, COL_COL2);
    draw_turret(x + 78, y - 8, 10, 13, enemy[e].aim[1], 8, 3, COL_COL4,
                COL_COL3, COL_COL2);

    /*           draw_turret(x - 55, y + 7, 10, 13, enemy[e].aim [0], 0, 0,
       COL_COL4, COL_COL3, COL_COL2); draw_turret(x - 106, y + 65, 9, 12,
       enemy[e].aim [0], 7, 2, COL_COL4, COL_COL3, COL_COL2);

               draw_turret(x + 55, y + 7, 10, 13, enemy[e].aim [0], 0, 0,
       COL_COL4, COL_COL3, COL_COL2); draw_turret(x + 106, y + 65, 9, 12,
       enemy[e].aim [1], 7, 2, COL_COL4, COL_COL3, COL_COL2);*/
  ...
  case ENEMY_MACHINE:
    ...
    draw_trans_rle_sprite(display, RLE_ccircle_basic[0][xa], x - xa - 1,
                          y - xa - 1 + 12);
    xa -= 1 + grand(2);
    draw_trans_rle_sprite(display, RLE_ccircle_4cols[0][xa], x - xa - 1,
                          y - xa - 1 + 16);
    xa -= 1 + grand(2);
    draw_trans_rle_sprite(display, RLE_ccircle_3cols[0][xa], x - xa - 1,
                          y - xa - 1 + 20);
    xa -= 1 + grand(2);
    draw_trans_rle_sprite(display, RLE_ccircle_3cols[0][xa], x - xa - 1,
                          y - xa - 1 + 24);
  ...
  case ENEMY_MACHINE2:
    ...
    xa = 6 + grand(4);
    draw_trans_rle_sprite(display, RLE_ccircle_basic[0][xa],
                          x - xa - 1 - 16 - enemy[e].frame, y - xa - 1 + 28);
    draw_trans_rle_sprite(display, RLE_ccircle_basic[0][xa],
                          x - xa - 1 + 16 + enemy[e].frame, y - xa - 1 + 28);
    xa -= 1 + grand(2);
    draw_trans_rle_sprite(display, RLE_ccircle_4cols[0][xa],
                          x - xa - 1 - 16 - enemy[e].frame, y - xa - 1 + 32);
    draw_trans_rle_sprite(display, RLE_ccircle_4cols[0][xa],
                          x - xa - 1 + 16 + enemy[e].frame, y - xa - 1 + 32);
    xa -= 1 + grand(2);
    draw_trans_rle_sprite(display, RLE_ccircle_3cols[0][xa],
                          x - xa - 1 - 16 - enemy[e].frame, y - xa - 1 + 36);
    draw_trans_rle_sprite(display, RLE_ccircle_3cols[0][xa],
                          x - xa - 1 + 16 + enemy[e].frame, y - xa - 1 + 36);
    xa -= 1 + grand(2);
    draw_trans_rle_sprite(display, RLE_ccircle_3cols[0][xa],
                          x - xa - 1 - 16 - enemy[e].frame, y - xa - 1 + 40);
    draw_trans_rle_sprite(display, RLE_ccircle_3cols[0][xa],
                          x - xa - 1 + 16 + enemy[e].frame, y - xa - 1 + 40);
    break;
  ...
  case ENEMY_MACHINE3:
    ...
    case 3:
      ...
      break;
      /*          case 1:
                 draw_rle_sprite(display, eRLE_small1 [S_ENEMY_MACHINE2_2], x -
         43, y - 21); break; case 2: draw_rle_sprite(display, eRLE_small1
         [S_ENEMY_MACHINE2_3], x - 44, y - 21); break; case 3:
                 draw_rle_sprite(display, eRLE_small1 [S_ENEMY_MACHINE2_4], x -
         45, y - 21); break;*/
    }
    xa = 5 + grand(3);
    draw_trans_rle_sprite(display, RLE_ccircle_basic[0][xa],
                          x - xa - 1 - 19 - enemy[e].frame,
                          y - xa - 1 + 44 + enemy[e].frame);
    draw_trans_rle_sprite(display, RLE_ccircle_basic[0][xa],
                          x - xa - 1 + 19 + enemy[e].frame,
                          y - xa - 1 + 44 + enemy[e].frame);
    xa -= 1 + grand(2);
  ...
  case ENEMY_MEDIUM:
    ...
    draw_trans_rle_sprite(display, RLE_ccircle_basic[2][xa], x - xa - 1,
                          y - xa - 1 + 12);
    xa -= 1 + grand(2);
    draw_trans_rle_sprite(display, RLE_ccircle_4cols[2][xa], x - xa - 1,
                          y - xa - 1 + 16);
    xa -= 1 + grand(2);
    draw_trans_rle_sprite(display, RLE_ccircle_3cols[2][xa], x - xa - 1,
                          y - xa - 1 + 21);
    break;
  case ENEMY_FIGHTER4:
    xa = 4 + grand(3);
    draw_trans_rle_sprite(display, RLE_ccircle_basic[0][xa], x - xa - 1 - 7,
                          y - xa - 1 + 19);
    draw_trans_rle_sprite(display, RLE_ccircle_basic[0][xa], x - xa - 1 + 7,
                          y - xa - 1 + 19);
    xa -= 1 + grand(2);
    draw_trans_rle_sprite(display, RLE_ccircle_4cols[0][xa], x - xa - 1 - 7,
                          y - xa - 1 + 23);
    draw_trans_rle_sprite(display, RLE_ccircle_4cols[0][xa], x - xa - 1 + 7,
                          y - xa - 1 + 23);
    xa -= 1 + grand(2);
    draw_trans_rle_sprite(display, RLE_ccircle_3cols[0][xa], x - xa - 1 - 7,
                          y - xa - 1 + 27);
    draw_trans_rle_sprite(display, RLE_ccircle_3cols[0][xa], x - xa - 1 + 7,
                          y - xa - 1 + 27);
    ...
    if (enemy[e].recycle < 45) {
      ...
      for (i = 0; i < 8; i++) {
        ...
        bcircle4(x + xpart(angle, xa), y + 4 + ypart(angle, xa),
                 5 - enemy[e].recycle / 10, CLOUD_COL_3);
      }
      angle = ANGLE_1 - angle;
      for (i = 0; i < 8; i++) {
        angle += ANGLE_8;
      ...
      //          draw_trans_rle_sprite(display, RLE_ccircle_3cols [0] [47], x -
      //          xa - 1 - 7, y - xa - 1 + 27);
    }
    break;
  case ENEMY_FIGHTER3:
    ...
    switch (enemy[e].frame) {
    default:
      draw_rle_sprite(display, eRLE_small1[S_ENEMY_FIGHTER3], x - 17, y - 10);
      break;
    case 1:
      draw_rle_sprite(display, eRLE_small1[S_ENEMY_FIGHTER3_2], x - 18, y - 10);
      break;
    case 2:
      draw_rle_sprite(display, eRLE_small1[S_ENEMY_FIGHTER3_3], x - 19, y - 10);
      break;
    }
  ...
  case ENEMY_FIGHTER2:
    switch (enemy[e].frame) {
    default:
      draw_rle_sprite(display, eRLE_small1[S_ENEMY_FIGHTER2], x - 20, y - 14);
      break;
    case 1:
      draw_rle_sprite(display, eRLE_small1[S_ENEMY_FIGHTER2_2], x - 20, y - 14);
      break;
    case 2:
      draw_rle_sprite(display, eRLE_small1[S_ENEMY_FIGHTER2_3], x - 20, y - 14);
      break;
    case 3:
      draw_rle_sprite(display, eRLE_small1[S_ENEMY_FIGHTER2_4], x - 21, y - 14);
      break;
    }
    ...
    /*         if (enemy[e].status > 0)
             {
              ...
              draw_trans_rle_sprite(display, RLE_ccircle_basic [CLOUD_COL_2]
       [xa], x - xa - 1, y - xa - 1);
             }*/
    break;
  case ENEMY_FIGHTER:
    switch (enemy[e].frame) {
    default:
      draw_rle_sprite(display, eRLE_small1[S_ENEMY_FIGHTER], x - 14, y - 13);
      break;
    case 1:
      draw_rle_sprite(display, eRLE_small1[S_ENEMY_FIGHTER_2], x - 14, y - 13);
      break;
    case 2:
      draw_rle_sprite(display, eRLE_small1[S_ENEMY_FIGHTER_3], x - 14, y - 13);
      break;
    }
    ...
    if (enemy[e].status > 0) {
      xa = 1 + enemy[e].status / 3 + grand(4);
      draw_trans_rle_sprite(display, RLE_ccircle_basic[CLOUD_COL_2][xa],
                            x - xa - 1, y - xa - 1);
    }
    break;
  case ENEMY_FIGHTER5:
    ...
    draw_trans_rle_sprite(display, RLE_ccircle_3cols[0][xa], x - xa - 1,
                          y - xa - 1 + 12);
    //        line(display, x, y, enemy[e].x_target [0] >> 10, enemy[e].y_target
    //        [0] >> 10, COL_WHITE); pline(display, enemy[e].x_target [0] >> 10,
    //        enemy[e].y_target [0] >> 10, enemy[e].x_target [1] >> 10,
    //        enemy[e].y_target [1] >> 10, COL_WHITE);
    // pline(display, enemy[e].x_target [1] >> 10, enemy[e].y_target [1] >> 10,
    // enemy[e].x_target [2] >> 10, enemy[e].y_target [2] >> 10, COL_WHITE);
    break;
  ...
  case ENEMY_FIGHTER6:
    /*         switch(enemy[e].frame)
             {
              default:
               draw_rle_sprite(display, eRLE_small1 [S_ENEMY_FIGHTER5], x - 19,
       y - 17); break; case 1: draw_rle_sprite(display, eRLE_small1
       [S_ENEMY_FIGHTER5_2], x - 20, y - 17); break; case 2:
               draw_rle_sprite(display, eRLE_small1 [S_ENEMY_FIGHTER5_3], x -
       21, y - 17); break;
             }*/
    ...
    draw_trans_rle_sprite(display, RLE_ccircle_basic[2][xa], x - xa - 1,
                          y - xa - 1 + 18);
    //         draw_trans_rle_sprite(display, RLE_ccircle_basic [2] [xa], x - xa
    //         - 1 - 12, y - xa - 1 + 20); draw_trans_rle_sprite(display,
    //         RLE_ccircle_basic [2] [xa], x - xa - 1 + 12, y - xa - 1 + 20);
    xa -= 1 + grand(2);
    draw_trans_rle_sprite(display, RLE_ccircle_4cols[2][xa], x - xa - 1,
                          y - xa - 1 + 23);
    draw_trans_rle_sprite(display, RLE_ccircle_4cols[2][xa], x - xa - 1 - 12,
                          y - xa - 1 + 18);
    draw_trans_rle_sprite(display, RLE_ccircle_4cols[2][xa], x - xa - 1 + 12,
                          y - xa - 1 + 18);
    xa -= 1 + grand(2);
    draw_trans_rle_sprite(display, RLE_ccircle_3cols[2][xa], x - xa - 1,
                          y - xa - 1 + 27);
    draw_trans_rle_sprite(display, RLE_ccircle_3cols[2][xa], x - xa - 1 - 12,
                          y - xa - 1 + 21);
    draw_trans_rle_sprite(display, RLE_ccircle_3cols[2][xa], x - xa - 1 + 12,
                          y - xa - 1 + 21);
    //        line(display, x, y, enemy[e].x_target [0] >> 10, enemy[e].y_target
  ...
  case ENEMY_FIGHTER7:
    ...
    xa = 4 + grand(4);
    draw_trans_rle_sprite(display, RLE_ccircle_basic[2][xa], x - xa - 1 - 10,
                          y - xa - 1 + 17);
    xa -= 1 + grand(2);
    draw_trans_rle_sprite(display, RLE_ccircle_4cols[2][xa], x - xa - 1 - 10,
                          y - xa - 1 + 21);
    xa -= 1 + grand(2);
    draw_trans_rle_sprite(display, RLE_ccircle_3cols[2][xa], x - xa - 1 - 10,
                          y - xa - 1 + 25);
    ...
    /*         xa = 4 + grand(4);
    draw_trans_rle_sprite(display, RLE_ccircle_basic [2] [xa], x - xa - 1 - 10,
    y - xa - 1 + 17); draw_trans_rle_sprite(display, RLE_ccircle_basic [2] [xa],
    x - xa - 1 + 10, y - xa - 1 + 17); xa -= 1 + grand(2);
    draw_trans_rle_sprite(display, RLE_ccircle_4cols [2] [xa], x - xa - 1 - 10,
    y - xa - 1 + 21); draw_trans_rle_sprite(display, RLE_ccircle_4cols [2] [xa],
    x - xa - 1 + 10, y - xa - 1 + 21); xa -= 1 + grand(2);
    draw_trans_rle_sprite(display, RLE_ccircle_3cols [2] [xa], x - xa - 1 - 10,
    y - xa - 1 + 25); draw_trans_rle_sprite(display, RLE_ccircle_3cols [2] [xa],
    x - xa - 1 + 10, y - xa - 1 + 25);*/
    break;
  ...
  case ENEMY_DOUBLE3:
    ...
    draw_turret(x - 18, y - 17, 7, 9, enemy[e].aim[0], 5, 3, COL_COL4, COL_COL3,
                COL_COL2);
    draw_turret(x + 18, y - 17, 7, 9, enemy[e].aim[1], 5, 3, COL_COL4, COL_COL3,
                COL_COL2);

    xa = 8 + grand(5);
    draw_trans_rle_sprite(display, RLE_ccircle_basic[0][xa], x - xa - 1 + 42,
                          y - xa - 1 + 35);
    draw_trans_rle_sprite(display, RLE_ccircle_basic[0][xa], x - xa - 1 - 42,
                          y - xa - 1 + 35);
    xa -= 2 + grand(2);
  ...
  case ENEMY_DOME3:
    ...
    for (i = 0; i < 10; i++) {
      ...
      circlefill(display, x + xpart(angle, 19), y - 19 + ypart(angle, 19), 3,
                 COL_COL4);
      circle(display, x + xpart(angle, 19), y - 19 + ypart(angle, 19), 3,
             COL_COL3);
  ...
  case ENEMY_DOUBLE4:
    ...
    draw_turret(x - 69, y + 19, 7, 9, enemy[e].aim[0], 5, 3, COL_COL4, COL_COL3,
                COL_COL2);
    draw_turret(x + 69, y + 19, 7, 9, enemy[e].aim[1], 5, 3, COL_COL4, COL_COL3,
                COL_COL2);

    xa = 8 + grand(5);
    draw_trans_rle_sprite(display, RLE_ccircle_basic[0][xa], x - xa - 1 + 44,
                          y - xa - 1 + 60);
    draw_trans_rle_sprite(display, RLE_ccircle_basic[0][xa], x - xa - 1 - 44,
                          y - xa - 1 + 60);
    xa -= 2 + grand(2);
  ...
  case ENEMY_DOUBLE2:
    ...
    circlefill(display, x - 19 + xpart(enemy[e].aim[0], 6),
               y - 10 + ypart(enemy[e].aim[0], 6), 2, COL_COL4);
    circle(display, x - 19 + xpart(enemy[e].aim[0], 6),
           y - 10 + ypart(enemy[e].aim[0], 6), 2, COL_COL2);

    circlefill(display, x + 19, y - 10, 10, COL_COL3);
    circle(display, x + 19, y - 10, 10, COL_COL2);
    circlefill(display, x + 19, y - 10, 7, COL_COL4);
    circle(display, x + 19, y - 10, 7, COL_COL2);
    circlefill(display, x + 19 + xpart(enemy[e].aim[1], 6),
               y - 10 + ypart(enemy[e].aim[1], 6), 2, COL_COL4);
    circle(display, x + 19 + xpart(enemy[e].aim[1], 6),
           y - 10 + ypart(enemy[e].aim[1], 6), 2, COL_COL2);

    xa = 8 + grand(5);
    draw_trans_rle_sprite(display, RLE_ccircle_basic[0][xa], x - xa - 1 + 43,
                          y - xa - 1 + 31);
    draw_trans_rle_sprite(display, RLE_ccircle_basic[0][xa], x - xa - 1 - 43,
                          y - xa - 1 + 31);
    xa -= 2 + grand(2);
  ...
  case ENEMY_DOUBLE:
    ...
    circlefill(display, x - 21 + xpart(enemy[e].aim[0], 4),
               y + 10 + ypart(enemy[e].aim[0], 4), 2, COL_COL4);
    circle(display, x - 21 + xpart(enemy[e].aim[0], 4),
           y + 10 + ypart(enemy[e].aim[0], 4), 2, COL_COL2);
    /*           angle = (enemy[e].aim [0] / 4) & 0xff;
               xa = RLE_barrel1 [angle].x;
               ya = RLE_barrel1 [angle].y;
               draw_rle_sprite(display, RLE_barrel1 [angle].sprite, x - 21 +
       xpart(enemy[e].aim [0], 7) - xa, y + 10 + ypart(enemy[e].aim [0], 7) -
       ya);
         //textprintf_centre_ex(display, font, x, y, -1, -1, "%i, %i, %i",
       angle, xa, ya); circlefill(display, x - 21, y + 10, 5, COL_COL4);
               circle(display, x - 21, y + 10, 5, COL_COL1);*/

    circlefill(display, x + 21, y + 10, 5, COL_COL4);
    circle(display, x + 21, y + 10, 5, COL_COL2);
    circlefill(display, x + 21 + xpart(enemy[e].aim[1], 4),
               y + 10 + ypart(enemy[e].aim[1], 4), 2, COL_COL4);
    circle(display, x + 21 + xpart(enemy[e].aim[1], 4),
           y + 10 + ypart(enemy[e].aim[1], 4), 2, COL_COL2);
  ...
  case ENEMY_RISER:
    ...
    draw_trans_rle_sprite(display, RLE_ccircle_basic[2][xa], x - xa - 1 - 16,
                          y - xa - 1 + 16);
    draw_trans_rle_sprite(display, RLE_ccircle_basic[2][xa], x - xa - 1 + 16,
                          y - xa - 1 + 16);
    xa -= 1 + grand(2);
    if (xa > 1) {
      draw_trans_rle_sprite(display, RLE_ccircle_4cols[2][xa], x - xa - 1 - 19,
                            y - xa - 1 + 19);
      draw_trans_rle_sprite(display, RLE_ccircle_4cols[2][xa], x - xa - 1 + 19,
                            y - xa - 1 + 19);
      xa -= 1 + grand(2);
      if (xa > 1) {
        draw_trans_rle_sprite(display, RLE_ccircle_3cols[2][xa],
                              x - xa - 1 - 22, y - xa - 1 + 22);
        draw_trans_rle_sprite(display, RLE_ccircle_3cols[2][xa],
                              x - xa - 1 + 22, y - xa - 1 + 22);
      }
    ...
    /*         xa = 4 + grand(4);
             draw_trans_rle_sprite(display, RLE_ccircle_basic [2] [xa], x - xa -
       1, y - xa - 1 + 22); xa -= 1 + grand(2); draw_trans_rle_sprite(display,
       RLE_ccircle_4cols [2] [xa], x - xa - 1, y - xa - 1 + 26); xa -= 1 +
       grand(2); draw_trans_rle_sprite(display, RLE_ccircle_3cols [2] [xa], x -
       xa - 1, y - xa - 1 + 30);
    */
    break;
  }
...
#ifdef CBOXES
  rect(display,
       x - eclass[enemy[e].type].xsize / 1000 +
           eclass[enemy[e].type].xoffset / 1000,
       y - eclass[enemy[e].type].ysize / 1000 +
           eclass[enemy[e].type].yoffset / 1000,
       x + eclass[enemy[e].type].xsize / 1000 +
           eclass[enemy[e].type].xoffset / 1000,
       y + eclass[enemy[e].type].ysize / 1000 +
           eclass[enemy[e].type].yoffset / 1000,
...
void draw_turret(int x, int y, int d1, int d2, int angle, int angle_rad,
  ...
  if (angle_size > 0) {
    circlefill(display, x + xpart(angle, angle_rad),
               y + ypart(angle, angle_rad), angle_size, col1);
    circle(display, x + xpart(angle, angle_rad), y + ypart(angle, angle_rad),
           angle_size, col3);
  }
  //           circle(display, x - 76 + xpart(enemy[e].aim [0], 5), y - 11 +
  //           ypart(enemy[e].aim [0], 5), 3, COL_COL2);
}
...
void draw_an_ebullet(int b) {
  ...
  case EBULLET_BEAM:
    ...
    case 0:
      bcircle(x, y, xc + grand(8), CLOUD_COL_1);
      break;
    case 1:
      bcircle(x, y, xc + grand(8), CLOUD_COL_2);
      break;
    case 2:
      bcircle(x, y, xc + grand(8), CLOUD_COL_3);
      break;
    case 3:
      bcircle(x, y, xc + grand(8), CLOUD_COL_4);
      break;
    }
    ...
    poly4(display, x + xpart(yb + ANGLE_4, yc), y + ypart(yb + ANGLE_4, yc),
          ...
          x + xpart(yb + xc, xb), y + ypart(yb + xc, xb),
          beam_colours[ebullet[b].colour][0]);
    ...
    poly4(display, x + xpart(yb + ANGLE_4, yc), y + ypart(yb + ANGLE_4, yc),
          ...
          x + xpart(yb - xc, xb), y + ypart(yb - xc, xb),
          x + xpart(yb + xc, xb), y + ypart(yb + xc, xb),
          beam_colours[ebullet[b].colour][1]);
    ...
    if (xd > 10) {
      ...
      poly4(display, x + xpart(yb + ANGLE_4, yc), y + ypart(yb + ANGLE_4, yc),
            ...
            x + xpart(yb - xc, xb), y + ypart(yb - xc, xb),
            x + xpart(yb + xc, xb), y + ypart(yb + xc, xb),
            beam_colours[ebullet[b].colour][2]);
    }
    ...
    if (xd > 35) {
      ...
      poly4(display, x + xpart(yb + ANGLE_4, yc), y + ypart(yb + ANGLE_4, yc),
            ...
            x + xpart(yb - xc, xb), y + ypart(yb - xc, xb),
            x + xpart(yb + xc, xb), y + ypart(yb + xc, xb),
            beam_colours[ebullet[b].colour][3]);
    }
    ...
    if (xd > 45) {
      ...
      poly4(display, x + xpart(yb + ANGLE_4, yc), y + ypart(yb + ANGLE_4, yc),
            ...
            x + xpart(yb - xc, xb), y + ypart(yb - xc, xb),
            x + xpart(yb + xc, xb), y + ypart(yb + xc, xb),
            beam_colours[ebullet[b].colour][4]);
    }
    END_TRANS_MODE
    break;
  ...
  case EBULLET_BEAM2:
    ...
    poly4(display, x + xpart(yb + ANGLE_4, yc), y + ypart(yb + ANGLE_4, yc),
          x + xpart(yb - ANGLE_4, yc), y + ypart(yb - ANGLE_4, yc),
          x + xpart(yb, xb) + xpart(yb - ANGLE_4, yc),
          y + ypart(yb, xb) + ypart(yb - ANGLE_4, yc),
          x + xpart(yb, xb) + xpart(yb + ANGLE_4, yc),
          y + ypart(yb, xb) + xpart(yb + ANGLE_4, yc),
          beam_colours[ebullet[b].colour][0]);
    ...
    /*    case EBULLET_BEAM:
         ...
         poly4(display, x + xpart(yb + ANGLE_4, yc), y + ypart(yb + ANGLE_4,
       yc), x + xpart(yb - ANGLE_4, yc), y + ypart(yb - ANGLE_4, yc), x +
       xpart(yb - xc, xb), y + ypart(yb - xc, xb), x + xpart(yb + xc, xb), y +
       ypart(yb + xc, xb), TRANS_RED1 + TRANS_DIFFERENCE * ebullet[b].colour);
       ...
       yc), x + xpart(yb - ANGLE_4, yc), y + ypart(yb - ANGLE_4, yc), x +
       xpart(yb - xc, xb), y + ypart(yb - xc, xb), x + xpart(yb + xc, xb), y +
       ypart(yb + xc, xb), TRANS_RED2 + TRANS_DIFFERENCE * ebullet[b].colour);
       ...
       yc), x + xpart(yb - ANGLE_4, yc), y + ypart(yb - ANGLE_4, yc), x +
       xpart(yb - xc, xb), y + ypart(yb - xc, xb), x + xpart(yb + xc, xb), y +
       ypart(yb + xc, xb), TRANS_RED3 + TRANS_DIFFERENCE * ebullet[b].colour);
         }
       ...
       yc), x + xpart(yb - ANGLE_4, yc), y + ypart(yb - ANGLE_4, yc), x +
       xpart(yb - xc, xb), y + ypart(yb - xc, xb), x + xpart(yb + xc, xb), y +
       ypart(yb + xc, xb), TRANS_RED4 + TRANS_DIFFERENCE * ebullet[b].colour);
         }
       ...
       yc), x + xpart(yb - ANGLE_4, yc), y + ypart(yb - ANGLE_4, yc), x +
       xpart(yb - xc, xb), y + ypart(yb - xc, xb), x + xpart(yb + xc, xb), y +
       ypart(yb + xc, xb), TRANS_RED5 + TRANS_DIFFERENCE * ebullet[b].colour);
         }
         END_TRANS_MODE
         break;
         */
  case EBULLET_DRAGGER:
    ...
    bcircle(x - xpart(ebullet[b].angle, 2 + size),
            y - ypart(ebullet[b].angle, 2 + size), 3 + grand(3),
            ebullet[b].colour);
    bcircle(x - xpart(ebullet[b].angle, 4 + size),
            y - ypart(ebullet[b].angle, 4 + size), 2 + grand(3),
            ebullet[b].colour);
    break;
  ...
  case EBULLET_BLOBSHOT:
    ...
    bcircle(x - xpart(ebullet[b].angle, 4 + size),
            y - ypart(ebullet[b].angle, 4 + size), 3 + grand(4),
            ebullet[b].colour);
    bcircle(x + xpart(ebullet[b].angle, 4 + size),
            y + ypart(ebullet[b].angle, 4 + size), 2 + grand(4),
            ebullet[b].colour);
    break;
  ...
  case EBULLET_FLAKE:
    ...
    if (ebullet[b].timeout > 3980) {
      ...
      for (i = 0; i < 5; i++) {
        TRANS_MODE
        circlefill(display, x + xpart(xa, xb), y + ypart(xa, xb), size,
                   TRANS_BLUE3);
        END_TRANS_MODE
        draw_trans_rle_sprite(
            display, RLE_ccircle_basic[ebullet[b].colour][size],
            x - size - 1 + xpart(xa, xb), y - size - 1 + ypart(xa, xb));
        xa += ANGLE_5;
      }
      break;
    }
    ...
    for (i = 0; i < 5; i++) {
      draw_trans_rle_sprite(display, RLE_ibullet[ebullet[b].colour][size],
                            x + xpart(xa, 11) - 5, y + ypart(xa, 11) - 5);
      xa += ANGLE_5;
    }
    break;
  case EBULLET_FLAKE2:
    ...
    if (ebullet[b].timeout > 3980) {
      ...
      for (j = 0; j < 5; j++) {
        ya += ANGLE_5;
        circlefill(display, x + xpart(ANGLE_1 - ya, xb * 1.5),
                   y + ypart(ANGLE_1 - ya, xb * 1.5), size, TRANS_RED3);
        for (i = 0; i < 5; i++) {
          TRANS_MODE
          circlefill(display, x + xpart(ya, yb) + xpart(xa, xb),
                     y + ypart(ya, yb) + ypart(xa, xb), size, TRANS_BLUE3);
          END_TRANS_MODE
          draw_trans_rle_sprite(display,
                                RLE_ccircle_basic[ebullet[b].colour][size],
                                x - size - 1 + xpart(ya, yb) + xpart(xa, xb),
                                y - size - 1 + ypart(ya, yb) + ypart(xa, xb));
          xa += ANGLE_5;
        }
      }
      break;
    ...
    for (j = 0; j < 5; j++) {
      ya += ANGLE_5;
      draw_trans_rle_sprite(display, RLE_ibullet[0][size],
                            x + xpart(ANGLE_1 - ya, 16) - 5,
                            y + ypart(ANGLE_1 - ya, 16) - 5);
      for (i = 0; i < 5; i++) {
        draw_trans_rle_sprite(display, RLE_ibullet[ebullet[b].colour][size],
                              x + xpart(ya, 40) + xpart(xa, 11) - 5,
                              y + ypart(ya, 40) + ypart(xa, 11) - 5);
        xa += ANGLE_5;
      }
    }
    break;
  case EBULLET_FLAKE3:
    ...
    for (i = 0; i < 3; i++) {
      draw_trans_rle_sprite(display, RLE_ibullet[ebullet[b].colour][size],
                            x + xpart(xa, 13) - 5, y + ypart(xa, 13) - 5);
      xa += ANGLE_3;
    }
    break;
  case EBULLET_IBULLET:
    size = ebullet[b].status;
    draw_trans_rle_sprite(display, RLE_ibullet[ebullet[b].colour][size], x - 5,
                          y - 5);
    break;
  case EBULLET_IBULLET_2:
    size = ebullet[b].status;
    draw_trans_rle_sprite(display, RLE_ibullet_2[ebullet[b].colour][size],
                          x - 5, y - 5);
  ...
  case EBULLET_WIGGLY:
    ...
    draw_trans_rle_sprite(display, RLE_ccircle_basic[ebullet[b].colour][size],
                          x - size - 1, y - size - 1);
    break;
  case EBULLET_BLOB:
    size = ebullet[b].status + grand(3);
  ...
  case EBULLET_SHOT:
    ...
    size -= 1 + grand(2);
    draw_trans_rle_sprite(display, RLE_ccircle_4cols[ebullet[b].colour][size],
                          x - size - 1, y - size - 1);
    x = (ebullet[b].x - xpart(ebullet[b].angle, 6000)) >> 10;
    y = (ebullet[b].y - ypart(ebullet[b].angle, 6000)) >> 10;
    size -= 1 + grand(2);
    draw_trans_rle_sprite(display, RLE_ccircle_3cols[ebullet[b].colour][size],
                          x - size - 1, y - size - 1);

    /*     size -= grand(2);
         draw_trans_rle_sprite(display, RLE_ccircle_4cols [ebullet[b].colour]
       [size], x - size - 1, y - size - 1); size -= grand(2);
         draw_trans_rle_sprite(display, RLE_ccircle_3cols [ebullet[b].colour]
       [size], x - size - 1, y - size - 1); size -= grand(2) + 1; if (size < 0)
          size = 0;
         draw_trans_rle_sprite(display, RLE_ccircle_3cols [ebullet[b].colour]
       [size], x - size - 1, y - size - 1); size -= grand(2) + 1; if (size < 0)
          size = 0;
  ...
  case EBULLET_B13SHOT:
    ...
    for (i = 0; i < 3; i++) {
      ...
      bcircle(x + xpart(xa - (ebullet[b].status * 6), 16),
              y + ypart(xa - (ebullet[b].status * 6), 16), 2 + grand(3),
              ebullet[b].colour);
      xa += ANGLE_3;
    }
    break;
  case EBULLET_SPINNER3:
    ...
    for (i = 0; i < 3; i++) {
      ...
      //      bcircle(x + xpart(xa - (ebullet[b].status * 6), 16), y + ypart(xa
      //      - (ebullet[b].status * 6), 16), 2 + grand(3), ebullet[b].colour);
      xa += ANGLE_3;
    }
    break;
  case EBULLET_STOPMINE:
    ...
    xa = ebullet[b].angle;
    for (i = 0; i < 3; i++) {
      bcircle(x + xpart(xa, 12), y + ypart(xa, 12), 6,
              CLOUD_COL_4); // ebullet[b].colour);
      xa += ANGLE_3;
    }
    break;
  }
...
void draw_a_pbullet(int b) {
  ...
  case PBULLET_RINGSHOT:
    ...
    bcircle(x - xpart(pbullet[b].angle, 5 + size),
            y - ypart(pbullet[b].angle, 5 + size),
            4 + grand(4) + grand(pbullet[b].level / 2), 0);
    bcircle(x - xpart(pbullet[b].angle, 10 + size),
            y - ypart(pbullet[b].angle, 10 + size),
            3 + grand(3) + grand(pbullet[b].level / 2), 0);
    break;
  case PBULLET_ORBSHOT:
    size = 4 + pbullet[b].level / 3;
    bcircle(x, y, 4 + grand(2) + grand(pbullet[b].level / 2), CLOUD_COL_3);
    bcircle3(x - xpart(pbullet[b].angle, 1), y - ypart(pbullet[b].angle, 1),
             4 + grand(4) + grand(pbullet[b].level / 2), 2);
    bcircle3(x - xpart(pbullet[b].angle, 1 + size),
             y - ypart(pbullet[b].angle, 1 + size),
             3 + grand(3) + grand(pbullet[b].level / 2), 2);
    break;
  case PBULLET_HSHOT:
    ...
    bcircle(x - xpart(pbullet[b].angle, 5 + size),
            y - ypart(pbullet[b].angle, 5 + size),
            4 + grand(2) + grand(pbullet[b].level / 3), 0);
    bcircle(x - xpart(pbullet[b].angle, 10 + size),
            y - ypart(pbullet[b].angle, 10 + size),
            3 + grand(2) + grand(pbullet[b].level / 4), 0);
    /*     bcircle(x, y, 4 + grand(4), CLOUD_COL_1);
         bcircle4(x - xpart(pbullet[b].angle, 5), y - ypart(pbullet[b].angle,
       5), 4 + grand(4), 0); bcircle3(x - xpart(pbullet[b].angle, 10), y -
       ypart(pbullet[b].angle, 10), 3 + grand(3), 0);*/
    break;
  ...
  case PBULLET_SHOT:
    ...
    draw_trans_rle_sprite(display, RLE_ccircle_basic[0][size], x - size - 1,
                          y - size - 1);
    size -= grand(2);
    draw_trans_rle_sprite(display, RLE_ccircle_basic[0][size], x - size - 1,
                          y - size - 1 + 4);
    size -= grand(2);
    draw_trans_rle_sprite(display, RLE_ccircle_4cols[0][size], x - size - 1,
                          y - size - 1 + 8);
    size -= grand(2) + 1;
    if (size < 0)
      size = 0;
    draw_trans_rle_sprite(display, RLE_ccircle_3cols[0][size], x - size - 1,
                          y - size - 1 + 12);
    size -= grand(2) + 1;
    if (size < 0)
      size = 0;
  ...
  case PBULLET_ASHOT:
    ...
    break;
  case PBULLET_BLADE:
    // this is an awful hack. I shouldn't refer to player stats here but who can
    // stop me?
    ...
    //     TRANS_MODE
    /*        triangle(display,
                        (player.x + xpart(player.blade_angle,
       player.blade_distance)) >> 10, (player.y + ypart(player.blade_angle,
       player.blade_distance)) >> 10, (player.x + xpart(player.blade_angle,
       player.blade_distance) + xpart(player.blade_angle, player.blade_size)) >>
       10, (player.y + ypart(player.blade_angle, player.blade_distance) +
       ypart(player.blade_angle, player.blade_size)) >> 10, (player.x +
       xpart(player.blade_angle + ya, player.blade_distance) +
       xpart(player.blade_angle + ya, player.blade_size >> 1)) >> 10, (player.y
       + ypart(player.blade_angle + ya, player.blade_distance) +
       ypart(player.blade_angle + ya, player.blade_size >> 1)) >> 10,
                        TRANS_BLUE5);

            triangle(display,
                        (player.x + xpart(player.blade_angle - yb,
       player.blade_distance - 1000)) >> 10, (player.y +
       ypart(player.blade_angle - yb, player.blade_distance - 1000)) >> 10,
                        (player.x + xpart(player.blade_angle - yb,
       player.blade_distance + 5000) + xpart(player.blade_angle,
       player.blade_size)) >> 10, (player.y + ypart(player.blade_angle - yb,
       player.blade_distance + 5000) + ypart(player.blade_angle,
       player.blade_size)) >> 10, (player.x + xpart(player.blade_angle + ya +
       yb, player.blade_distance) + xpart(player.blade_angle + ya + yb,
       player.blade_size >> 1)) >> 10, (player.y + ypart(player.blade_angle + ya
       + yb, player.blade_distance) + ypart(player.blade_angle + ya + yb,
       player.blade_size >> 1)) >> 10, TRANS_BLUE4);

                        yb *= 2;

            triangle(display,
                        (player.x + xpart(player.blade_angle - yb,
       player.blade_distance - 2000)) >> 10, (player.y +
       ypart(player.blade_angle - yb, player.blade_distance - 2000)) >> 10,
                        (player.x + xpart(player.blade_angle - yb,
       player.blade_distance + 8000) + xpart(player.blade_angle,
       player.blade_size)) >> 10, (player.y + ypart(player.blade_angle - yb,
       player.blade_distance + 8000) + ypart(player.blade_angle,
       player.blade_size)) >> 10, (player.x + xpart(player.blade_angle + ya +
       yb, player.blade_distance) + xpart(player.blade_angle + ya + yb,
       player.blade_size >> 1)) >> 10, (player.y + ypart(player.blade_angle + ya
       + yb, player.blade_distance) + ypart(player.blade_angle + ya + yb,
       player.blade_size >> 1)) >> 10, TRANS_BLUE3);*/
    /*
         poly4(display, (player.x + xpart(player.blade_angle,
       player.blade_distance) + xpart(player.blade_angle + ANGLE_4, 1000)) >>
       10, (player.y + ypart(player.blade_angle, player.blade_distance) +
       ypart(player.blade_angle + ANGLE_4, 1000)) >> 10, (player.x +
       xpart(player.blade_angle, player.blade_distance) +
       xpart(player.blade_angle - ANGLE_4, 1000)) >> 10, (player.y +
       ypart(player.blade_angle, player.blade_distance) +
       ypart(player.blade_angle - ANGLE_4, 1000)) >> 10,

                        (player.x + xpart(player.blade_angle,
       player.blade_distance) + xpart(player.blade_angle - ANGLE_4, 1000) +
       xpart(player.blade_angle, player.blade_size)) >> 10, (player.y +
       ypart(player.blade_angle, player.blade_distance) +
       ypart(player.blade_angle - ANGLE_4, 1000) + ypart(player.blade_angle,
       player.blade_size)) >> 10, (player.x + xpart(player.blade_angle,
       player.blade_distance) + xpart(player.blade_angle + ANGLE_4, 1000) +
       xpart(player.blade_angle, player.blade_size)) >> 10, (player.y +
       ypart(player.blade_angle, player.blade_distance) +
       ypart(player.blade_angle + ANGLE_4, 1000) + ypart(player.blade_angle,
       player.blade_size)) >> 10, TRANS_BLUE4);

         poly4(display, (player.x + xpart(player.blade_angle,
       player.blade_distance) + xpart(player.blade_angle + ANGLE_4, 2000)) >>
       10, (player.y + ypart(player.blade_angle, player.blade_distance) +
       ypart(player.blade_angle + ANGLE_4, 2000)) >> 10, (player.x +
       xpart(player.blade_angle, player.blade_distance) +
       xpart(player.blade_angle - ANGLE_4, 2000)) >> 10, (player.y +
       ypart(player.blade_angle, player.blade_distance) +
       ypart(player.blade_angle - ANGLE_4, 2000)) >> 10, (player.x +
       xpart(player.blade_angle, player.blade_distance) +
       xpart(player.blade_angle - ANGLE_4, 2000) + xpart(player.blade_angle,
       player.blade_size)) >> 10, (player.y + ypart(player.blade_angle,
       player.blade_distance) + ypart(player.blade_angle - ANGLE_4, 2000) +
       ypart(player.blade_angle, player.blade_size)) >> 10, (player.x +
       xpart(player.blade_angle, player.blade_distance) +
       xpart(player.blade_angle + ANGLE_4, 2000) + xpart(player.blade_angle,
       player.blade_size)) >> 10, (player.y + ypart(player.blade_angle,
       player.blade_distance) + ypart(player.blade_angle + ANGLE_4, 2000) +
       ypart(player.blade_angle, player.blade_size)) >> 10, TRANS_BLUE3);

         poly4(display, (player.x + xpart(player.blade_angle,
       player.blade_distance) + xpart(player.blade_angle + ANGLE_4, 3000)) >>
       10, (player.y + ypart(player.blade_angle, player.blade_distance) +
       ypart(player.blade_angle + ANGLE_4, 3000)) >> 10, (player.x +
       xpart(player.blade_angle, player.blade_distance) +
       xpart(player.blade_angle - ANGLE_4, 3000)) >> 10, (player.y +
       ypart(player.blade_angle, player.blade_distance) +
       ypart(player.blade_angle - ANGLE_4, 3000)) >> 10, (player.x +
       xpart(player.blade_angle, player.blade_distance) +
       xpart(player.blade_angle - ANGLE_4, 3000) + xpart(player.blade_angle,
       player.blade_size)) >> 10, (player.y + ypart(player.blade_angle,
       player.blade_distance) + ypart(player.blade_angle - ANGLE_4, 3000) +
       ypart(player.blade_angle, player.blade_size)) >> 10, (player.x +
       xpart(player.blade_angle, player.blade_distance) +
       xpart(player.blade_angle + ANGLE_4, 3000) + xpart(player.blade_angle,
       player.blade_size)) >> 10, (player.y + ypart(player.blade_angle,
       player.blade_distance) + ypart(player.blade_angle + ANGLE_4, 3000) +
       ypart(player.blade_angle, player.blade_size)) >> 10, TRANS_BLUE2);
    ...
    bcircle((player.x +
             xpart(player.blade_angle - yb, player.blade_distance - 2000)) >>
                10,
            (player.y +
             ypart(player.blade_angle - yb, player.blade_distance - 2000)) >>
                10,
            1 + grand(5) + (xb / 6000), xc);

    bcircle((player.x +
             xpart(player.blade_angle - yb, player.blade_distance - 2000) +
             xpart(player.blade_angle, xb * 0.75)) >>
                10,
            (player.y +
             ypart(player.blade_angle - yb, player.blade_distance - 2000) +
             ypart(player.blade_angle, xb * 0.75)) >>
                10,
            1 + grand(6) + (xb / 5000), xc);

    bcircle((player.x +
             xpart(player.blade_angle - yb, player.blade_distance - 2000) +
             xpart(player.blade_angle, xb / 2)) >>
                10,
            (player.y +
             ypart(player.blade_angle - yb, player.blade_distance - 2000) +
             ypart(player.blade_angle, xb / 2)) >>
                10,
            1 + grand(8) + (xb / 4000), xc);

    bcircle((player.x +
             xpart(player.blade_angle - yb, player.blade_distance - 2000) +
             xpart(player.blade_angle, xb / 4)) >>
                10,
            (player.y +
             ypart(player.blade_angle - yb, player.blade_distance - 2000) +
             ypart(player.blade_angle, xb / 4)) >>
                10,
            1 + grand(6) + (xb / 5000), xc);

    bcircle((player.x +
             xpart(player.blade_angle - yb, player.blade_distance - 2000) +
             xpart(player.blade_angle, xb)) >>
                10,
            (player.y +
             ypart(player.blade_angle - yb, player.blade_distance - 2000) +
             ypart(player.blade_angle, xb)) >>
                10,
            1 + grand(5) + (xb / 6000), xc);

    /*     bcircle((player.x + xpart(player.blade_angle - yb,
       player.blade_distance - 2000)) >> 10, (player.y +
       ypart(player.blade_angle - yb, player.blade_distance - 2000)) >> 10, 1 +
       grand(5) + grand(player.blade_size / 6000), 2);

         bcircle((player.x + xpart(player.blade_angle - yb,
       player.blade_distance - 2000) + xpart(player.blade_angle,
       player.blade_size * 0.75)) >> 10, (player.y + ypart(player.blade_angle -
       yb, player.blade_distance - 2000) + ypart(player.blade_angle,
       player.blade_size * 0.75)) >> 10, 1 + grand(6) + grand(player.blade_size
       / 5000), 2);

         bcircle((player.x + xpart(player.blade_angle - yb,
       player.blade_distance - 2000) + xpart(player.blade_angle,
       player.blade_size / 2)) >> 10, (player.y + ypart(player.blade_angle - yb,
       player.blade_distance - 2000) + ypart(player.blade_angle,
       player.blade_size / 2))>> 10, 1 + grand(8) + grand(player.blade_size /
       4000), 2);

         bcircle((player.x + xpart(player.blade_angle - yb,
       player.blade_distance - 2000) + xpart(player.blade_angle,
       player.blade_size / 4)) >> 10, (player.y + ypart(player.blade_angle - yb,
       player.blade_distance - 2000) + ypart(player.blade_angle,
       player.blade_size / 4))>> 10, 1 + grand(6) + grand(player.blade_size /
       5000), 2);
       ...
       player.blade_size)) >> 10, (player.y + ypart(player.blade_angle - yb,
       player.blade_distance - 2000) + ypart(player.blade_angle,
       player.blade_size))>> 10, 1 + grand(5) + grand(player.blade_size / 6000),
       2);*/
...
void draw_a_cloud(int c) {
  ...
  case CLOUD_SHRINKING_BEAM:
    TRANS_MODE
    vline(display, x, y, cloud[c].colour >> 10,
          TRANS_RED2); // + grand(3) * 39);
    END_TRANS_MODE
  ...
  case CLOUD_SEEKER_TRAIL:
    ...
    do_line(display, x, y, cloud[c].x2 >> 10, cloud[c].y2 >> 10, size,
            draw_trail_line);
    //     END_TRANS
    break;
  ...
  case CLOUD_BLADE_TRAIL:
    ...
    if (size > 3)
      size = 3;
    TRANS_MODE
    pline(display, x, y, cloud[c].x2 >> 10, cloud[c].y2 >> 10,
          TRANS_RED1 + 39 * size);
    END_TRANS
    break;
  case CLOUD_CIRCLE1_2:
    ...
    if (size >= RLE_CCIRCLES)
      size = RLE_CCIRCLES - 1;
    draw_trans_rle_sprite(display, RLE_ccircle_basic[cloud[c].colour][size],
                          x - size - 1, y - size - 1);
    break;
  ...
  case CLOUD_FCIRCLE1_2:
    ...
    if (!grand(3))
      ...
      draw_trans_rle_sprite(display, RLE_ccircle_4cols[cloud[c].colour][size],
                            x - size - 1, y - size - 1);
    break;
  case CLOUD_FCIRCLE1:
    size = cloud[c].timeout + grand(3);
  ...
  case CLOUD_3FCIRCLE1_2:
    ...
    if (size >= RLE_CCIRCLES)
      size = RLE_CCIRCLES - 1;
    if (!grand(3))
      draw_trans_rle_sprite(display, RLE_ccircle_3cols[cloud[c].colour][size],
                            x - size - 1, y - size - 1);
    else
      draw_trans_rle_sprite(display, RLE_ccircle_4cols[cloud[c].colour][size],
                            x - size - 1, y - size - 1);
  ...
  case CLOUD_SMALL_SHOCK:
    ...
    if (size < 0)
      size = 0;
    draw_trans_rle_sprite(display, RLE_small_shock[cloud[c].colour][size],
                          x - size - 16, y - size - 16);
    break;
  ...
  case CLOUD_LARGE_SHOCK:
    ...
    if (size < 0)
      size = 0;
    draw_trans_rle_sprite(display, RLE_large_shock[cloud[c].colour][size],
                          x - size - 26, y - size - 26);
    break;
  ...
  case CLOUD_HUGE_SHOCK_FAST:
    ...
    if (size < 0)
      size = 0;
    draw_trans_rle_sprite(display, RLE_huge_shock[cloud[c].colour][size],
                          x - size - 51, y - size - 51);
    break;
...
void draw_background(void) {
  ...
  case 1:
    for (i = 0; i < NO_BCIRCLES; i++) {
      ...
      if (size > 31)
        size = 31;
      draw_trans_rle_sprite(display, RLE_bcircle[0][size],
                            (bcircle_x[0][i] >> 10) - size,
                            (bcircle_y[0][i] >> 10) - size);
      //     textprintf_centre_ex(display, font, bcircle_x [0] [i] >> 10,
      //     bcircle_y [0] [i] >> 10, -1, -1, "%i %i", size, i);
    }
    ...
    for (i = 0; i < NO_BCIRCLES; i++) {
      ...
      if (size > 31)
        size = 31;
      draw_trans_rle_sprite(display, RLE_bcircle[1][size],
                            (bcircle_x[1][i] >> 10) - size,
                            (bcircle_y[1][i] >> 10) - size);
    }
    ...
    for (i = 0; i < NO_BCIRCLES; i++) {
      ...
      if (size > 31)
        size = 31;
      draw_trans_rle_sprite(display, RLE_bcircle[2][size],
                            (bcircle_x[2][i] >> 10) - size,
                            (bcircle_y[2][i] >> 10) - size);
    }
    break;
  case 2:
    for (i = 0; i < NO_BCIRCLES; i++) {
      if (panel_x_size[0][i] <= 0 ||
        ...
        continue;
      rectfill(display, (panel_x[0][i] >> 10) - panel_x_size[0][i],
               (panel_y[0][i] >> 10) - panel_y_size[0][i],
               (panel_x[0][i] >> 10) + panel_x_size[0][i],
               (panel_y[0][i] >> 10) + panel_y_size[0][i], COL_BACK2);
      TRANS_MODE
      rect(display, (panel_x[0][i] >> 10) - panel_x_size[0][i] - 1,
           (panel_y[0][i] >> 10) - panel_y_size[0][i] - 1,
           (panel_x[0][i] >> 10) + panel_x_size[0][i] + 1,
           (panel_y[0][i] >> 10) + panel_y_size[0][i] + 1, COL_BCIRCLE_OUT_1);
      END_TRANS
    }
    for (i = 0; i < NO_BCIRCLES; i++) {
      if (panel_x_size[1][i] <= 0 ||
        ...
        continue;
      rectfill(display, (panel_x[1][i] >> 10) - panel_x_size[1][i],
               (panel_y[1][i] >> 10) - panel_y_size[1][i],
               (panel_x[1][i] >> 10) + panel_x_size[1][i],
               (panel_y[1][i] >> 10) + panel_y_size[1][i], COL_BACK3);
      TRANS_MODE
      rect(display, (panel_x[1][i] >> 10) - panel_x_size[1][i] - 1,
           (panel_y[1][i] >> 10) - panel_y_size[1][i] - 1,
           (panel_x[1][i] >> 10) + panel_x_size[1][i] + 1,
           (panel_y[1][i] >> 10) + panel_y_size[1][i] + 1, COL_BCIRCLE_OUT_2);
      END_TRANS
    }
    for (i = 0; i < NO_BCIRCLES; i++) {
      if (panel_x_size[2][i] <= 0 ||
        ...
        continue;
      rectfill(display, (panel_x[2][i] >> 10) - panel_x_size[2][i],
               (panel_y[2][i] >> 10) - panel_y_size[2][i],
               (panel_x[2][i] >> 10) + panel_x_size[2][i],
               (panel_y[2][i] >> 10) + panel_y_size[2][i], COL_BACK4);
      TRANS_MODE
      rect(display, (panel_x[2][i] >> 10) - panel_x_size[2][i] - 1,
           (panel_y[2][i] >> 10) - panel_y_size[2][i] - 1,
           (panel_x[2][i] >> 10) + panel_x_size[2][i] + 1,
           (panel_y[2][i] >> 10) + panel_y_size[2][i] + 1, COL_BCIRCLE_OUT_3);
      END_TRANS
    }
    break;
  ...
  case 3:
    for (i = 0; i < NO_BCIRCLES; i++) {
      ...
      rectfill(display, (panel_x[0][i] >> 10) - (panel_x_size[0][i] >> 10),
               (panel_y[0][i] >> 10) - (panel_y_size[0][i] >> 10),
               (panel_x[0][i] >> 10) + (panel_x_size[0][i] >> 10),
               (panel_y[0][i] >> 10) + (panel_y_size[0][i] >> 10), COL_BACK2);
      TRANS_MODE
      rect(display, (panel_x[0][i] >> 10) - (panel_x_size[0][i] >> 10) - 1,
           (panel_y[0][i] >> 10) - (panel_y_size[0][i] >> 10) - 1,
           (panel_x[0][i] >> 10) + (panel_x_size[0][i] >> 10) + 1,
           (panel_y[0][i] >> 10) + (panel_y_size[0][i] >> 10) + 1,
           COL_BCIRCLE_OUT_1);
      END_TRANS
    }
    for (i = 0; i < NO_BCIRCLES; i++) {
      if (panel_x_size[1][i] <= 0 ||
          (panel_y[1][i] >> 10) + panel_y_size[1][i] < -1)
        continue;
      rectfill(display, (panel_x[1][i] >> 10) - (panel_x_size[1][i] >> 10),
               (panel_y[1][i] >> 10) - (panel_y_size[1][i] >> 10),
               (panel_x[1][i] >> 10) + (panel_x_size[1][i] >> 10),
               (panel_y[1][i] >> 10) + (panel_y_size[1][i] >> 10), COL_BACK3);
      TRANS_MODE
      rect(display, (panel_x[1][i] >> 10) - (panel_x_size[1][i] >> 10) - 1,
           (panel_y[1][i] >> 10) - (panel_y_size[1][i] >> 10) - 1,
           (panel_x[1][i] >> 10) + (panel_x_size[1][i] >> 10) + 1,
           (panel_y[1][i] >> 10) + (panel_y_size[1][i] >> 10) + 1,
           COL_BCIRCLE_OUT_2);
      END_TRANS
    }
    for (i = 0; i < NO_BCIRCLES; i++) {
      if (panel_x_size[2][i] <= 0 ||
          (panel_y[2][i] >> 10) + panel_y_size[2][i] < -1)
        continue;
      rectfill(display, (panel_x[2][i] >> 10) - (panel_x_size[2][i] >> 10),
               (panel_y[2][i] >> 10) - (panel_y_size[2][i] >> 10),
               (panel_x[2][i] >> 10) + (panel_x_size[2][i] >> 10),
               (panel_y[2][i] >> 10) + (panel_y_size[2][i] >> 10), COL_BACK4);
      TRANS_MODE
      rect(display, (panel_x[2][i] >> 10) - (panel_x_size[2][i] >> 10) - 1,
           (panel_y[2][i] >> 10) - (panel_y_size[2][i] >> 10) - 1,
           (panel_x[2][i] >> 10) + (panel_x_size[2][i] >> 10) + 1,
           (panel_y[2][i] >> 10) + (panel_y_size[2][i] >> 10) + 1,
           COL_BCIRCLE_OUT_3);
      END_TRANS
    }
...
void display_pause(int pc) {
  ...
  textprintf_centre_ex(display, font, 300, y1 + 100, -1, -1,
                       "PRESS 'Y' TO QUIT");
  textprintf_centre_ex(display, font, 300, y1 + 140, -1, -1,
                       "OR 'N' TO CONTINUE");
...
*/
void pline(BITMAP *bmp, int x1, int y1, int x2, int y2, int colour) {
  if (x1 < -500 || x1 > 900 || x2 < -500 || x2 > 900 || y2 < -500 || y2 > 900 ||
      y2 < -500 || y2 > 900)
    return;
...
void draw_bright_ring(int x, int y, int size) {
  ...
  draw_trans_rle_sprite(display, RLE_bright_ring[size], x - size - 4,
                        y - size - 4);
}
...
void bcircle(int x, int y, int size, unsigned char col) {
  ...
  draw_trans_rle_sprite(display, RLE_ccircle_basic[col][size], x - size - 1,
                        y - size - 1);
}
...
void bcircle4(int x, int y, int size, unsigned char col) {
  ...
  draw_trans_rle_sprite(display, RLE_ccircle_4cols[col][size], x - size - 1,
                        y - size - 1);
}
...
void bcircle3(int x, int y, int size, unsigned char col) {
  ...
  draw_trans_rle_sprite(display, RLE_ccircle_3cols[col][size], x - size - 1,
                        y - size - 1);
}
