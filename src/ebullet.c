...
/*
Creates an enemy bullet of type type and returns the index in the ebullet struct
array.
*/
  ...
  for (b = 0; b < NO_EBULLETS; b++) {
    ...
    switch (ebullet[b].type) {
      /*        case EBULLET_TRACKER:
         ...
         ebullet[b].timeout < 490) ebullet[b].angle =
    ...
    case EBULLET_BEAM:
      ...
      no_more_bullet = 1; // don't want to destroy the beam just because it
                          // starts outside the screen border.
      // here no_more_bullet just tells it not to run the destruction checks,
    ...
    case EBULLET_STOPMINE:
      ...
      if (ebullet[b].timeout <= 5) {
        quick_cloud(CLOUD_CIRCLE2, ebullet[b].x, ebullet[b].y, 0, 0, 50,
                    ebullet[b].colour, 0);
        no_more_bullet = 1;
        destroy_ebullet(b);
      }
    ...
    case EBULLET_DRAGGER:
      ...
      drag_ebullet(b, 995);
      if (ebullet[b].timeout <= 5) {
        quick_cloud(CLOUD_3FCIRCLE1, ebullet[b].x, ebullet[b].y, 0, 0, 15,
                    ebullet[b].colour, 0);
        no_more_bullet = 1;
        destroy_ebullet(b);
      }
      ...
      if (ebullet[b].status > 0) {
        if (ebullet[b].status & 1) {
          quick_cloud(CLOUD_FCIRCLE4, ebullet[b].x, ebullet[b].y,
                      ebullet[b].x_speed + grand(1000) - grand(1000),
                      ebullet[b].y_speed + grand(1000) - grand(1000),
                      10 + grand(10) + ebullet[b].status, ebullet[b].colour,
                      900);
        }
        ebullet[b].status--;
      }
    ...
    case EBULLET_SHOT2:
      ...
      //            {
      quick_cloud(CLOUD_3FCIRCLE8, ebullet[b].x, ebullet[b].y,
                  ebullet[b].x_speed, ebullet[b].y_speed, 20 + grand(30),
                  ebullet[b].colour, 920);
      //             quick_cloud(CLOUD_FCIRCLE8, ebullet[b].x, ebullet[b].y, 0,
      //             0, 20 + grand(30), ebullet[b].colour, 0);
      //            }
      break;
    case EBULLET_B21SHOT:
      ...
      drag_ebullet(b, 1010);
      quick_cloud(CLOUD_3FCIRCLE8, ebullet[b].x, ebullet[b].y,
                  ebullet[b].x_speed, ebullet[b].y_speed, 15 + grand(20),
                  ebullet[b].colour, 940);
      break;
    case EBULLET_WIGGLY:
      ebullet[b].x += ebullet[b].x_speed;
      ebullet[b].y += ebullet[b].y_speed;
      ...
      //            quick_cloud(CLOUD_3PCIRCLE2, ebullet[b].x, ebullet[b].y,
      //            ebullet[b].x_speed + xpart(angle, k), ebullet[b].y_speed +
      //            ypart(angle, k), 20, ebullet[b].colour, 980);
      quick_cloud(CLOUD_3PCIRCLE2, ebullet[b].x, ebullet[b].y, xpart(angle, k),
                  ypart(angle, k), 12, ebullet[b].colour, 940);
      break;
    case EBULLET_FLAKE:
    case EBULLET_FLAKE2:
    case EBULLET_FLAKE3:
    ...
    case EBULLET_B13SHOT:
      ...
      ebullet[b].angle += ebullet[b].status;
      ebullet[b].angle &= 1023;
      quick_cloud(CLOUD_3FCIRCLE2, ebullet[b].x + grand(8000) - grand(8000),
                  ebullet[b].y + grand(8000) - grand(8000), ebullet[b].x_speed,
                  ebullet[b].y_speed, 15 + grand(10), ebullet[b].colour, 960);
      break;
...
void destroy_ebullet(int b) { ebullet[b].type = EBULLET_NONE; }
