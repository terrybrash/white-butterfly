...
int find_enemies(int targets[TARGET_ARRAY_SIZE], int x, int y, int number,
                 int start_number, int force);
...
void get_input(void) {
  ...
  if (options.key_or_joy == 1) {
    ...
    //   if (player.swing_hold == 1 && joy[0].button
    //   [options.joy_button[player.blade_index]].b == 0)
    //    player.swing_hold = 0;

    /*   if (player.ring_hold == 1 && joy[0].button
       [options.joy_button[player.ring_index]].b == 0) player.ring_hold = 0;*/
    ...
    */
    ...
    xc = joy[0].stick[options.joy_stick].axis[0].pos *
         options.joy_sensitivity; // / 5120 * 6000;
    yc = joy[0].stick[options.joy_stick].axis[1].pos *
         options.joy_sensitivity; // / 5120 * 6000;
    ...
    */
    ...
    /*     if (player.ring_index != -1)
         {
          ...
          if (jangle > ANGLE_4 + ANGLE_8 + ANGLE_16)// && jangle <= ANGLE_8 +
       ANGLE_16)
          {
           player.move_x = -1;
           player.move_y = 0;
          ...
          if (jangle > ANGLE_2 + ANGLE_8 + ANGLE_16)// && jangle <= ANGLE_8 +
       ANGLE_16)
          {
           player.move_x = 0;
           player.move_y = -1;
          }
          if (jangle > ANGLE_2 + ANGLE_4 + ANGLE_16)// && jangle <= ANGLE_8 +
       ANGLE_16)
          {
           player.move_x = 1;
           player.move_y = -1;
          }
          if (jangle > ANGLE_1-ANGLE_16 || jangle < ANGLE_16)// && jangle <=
       ANGLE_16)
          {
           player.move_x = 1;
           player.move_y = 0;
    ...
    /* if (key [player.key [CKEY_FIRE1]])
     ...
     if (key [player.key [CKEY_FIRE3]])
       ...
       if (player.swing_hold == 1 && !key[player.key [CKEY_FIRE1 +
     player.blade_index]]) player.swing_hold = 0;

       if (player.ring_hold == 1 && !key[player.key [CKEY_FIRE1 +
     player.ring_index]]) player.ring_hold = 0;
  ...
  case PTYPE_BASIC:
    if (key1 && player.charge <= 0) {
      if (player.recycle1 <= 0) {
        play_effectwfvx(WAV_SHOT2, 1000 - player.weapon_level * 15, 80,
                        player.x);
        player.recoil = 9;
        player.recoiling = 1;
        ...
        for (f = 0; f < 2; f++) {
          ...
          if (b != -1) {
            ...
            pbullet[b].xsize =
                2000 +
                player.weapon_level * 660; // 9000 + player.wlevel [m] * 500;
            pbullet[b].ysize =
                2000 +
                player.weapon_level * 660; // 9000 + player.wlevel [m] * 500;
        ...
        player.wstrength[0] = ((pbullet[b].dam * 100) /
                               (player.recycle1 / player.recycle_speed1)) *
                              2;
#endif

        quick_cloud(CLOUD_CIRCLE1, player.x - 8000, player.y - 22000, 0, -200,
                    10 + grand(10), CLOUD_COL_1, 990);
        quick_cloud(CLOUD_CIRCLE1, player.x + 8000, player.y - 22000, 0, -200,
                    10 + grand(10), CLOUD_COL_1, 990);

        quick_cloud(CLOUD_CIRCLE4, player.x - 8000, player.y - 22000,
                    -1000 + grand(2000), -1000 - grand(2000), 10 + grand(25),
                    CLOUD_COL_1, 950);
        quick_cloud(CLOUD_CIRCLE4, player.x - 8000, player.y - 22000,
                    -1000 + grand(2000), -1000 - grand(2000), 10 + grand(25),
                    CLOUD_COL_1, 950);
...
#ifdef SHOW_PLAYER_DAMAGE
        player.wstrength[2] =
            (800 * (player.charge / 3000 - 1) * 50) / (player.charge / 200);
#endif
        quick_cloud(CLOUD_CIRCLE2, player.x, player.y - 16000, 0, 0,
                    15 + grand(20) + player.charge / 500, CLOUD_COL_1, 0);
        if (player.charge >= 20000)
          quick_cloud(CLOUD_SMALL_SHOCK, player.x, player.y - 16000, 0, 0, 30,
                      CLOUD_COL_1, 0);
        player.charge = 0;
      ...
      if (player.charge > 0) {
        if (player.charge > 6000) {
          ...
          if (b != -1) {
            ...
            pbullet[b].level = player.weapon_level;
            //         pbullet[b].dam = (player.charge / 2.5) - 1000; // if this
            //         changed, change in bullet explosion function also
            pbullet[b].dam =
                (player.charge * (10 + player.weapon_level)) /
                29; // if this changed, change in bullet explosion function also
            pbullet[b].dam -=
                1000; // there is some overhead for beginning the charge
            play_effectwfvx(WAV_CHARGE, 1500 - (player.charge) / 40, 140,
                            player.x);

#ifdef SHOW_PLAYER_DAMAGE
            player.wstrength[1] =
                (pbullet[b].dam * 100) / (player.charge / 200);
            //     player.charge / 200 is the number of ticks spent charging.
#endif
            pbullet[b].source = 1;
            quick_cloud(CLOUD_CIRCLE2, player.x, player.y - 16000, 0, 0,
                        20 + grand(20) + pbullet[b].dam / 200, CLOUD_COL_1, 0);
          }
        }
      }
      player.charge = 0;
  ...
  case PTYPE_ANENOME:
    if (key2 || key3) {
      ...
      if (player.blade_pos == BLADE_LEFT) {
        ...
        play_effectwfvx(WAV_SWEEP, 1000 - player.weapon_level * 15, 180,
                        player.x);
    ...
    if (key1) {
      if (player.recycle1 <= 0) {
        ...
        play_effectwfvx(WAV_SHOT2, 400 - player.weapon_level * 5, 100,
                        player.x);
        ...
        // don't bother testing for b here.
        player.wstrength[0] =
            (pbullet[b].dam * 25) / (player.recycle1 / player.recycle_speed1);
#endif

        quick_cloud(CLOUD_CIRCLE1, player.x, player.y - 12000, 0, -200,
                    20 + grand(10), CLOUD_COL_1, 990);

        quick_cloud(CLOUD_CIRCLE4, player.x, player.y - 12000,
                    -1000 + grand(2000), -1000 - grand(2000), 20 + grand(25),
                    CLOUD_COL_1, 960);
        quick_cloud(CLOUD_CIRCLE4, player.x, player.y - 12000,
                    -1000 + grand(2000), -1000 - grand(2000), 20 + grand(25),
                    CLOUD_COL_1, 960);
    ...
    if (!key1 || key2 || key3) {
      if (player.beam_firing != 0) {
        for (i = 0; i < 3; i++) {
          ...
          quick_cloud(CLOUD_SHRINKING_BEAM, x, y, 0, 0, 1000, player.beam_y[i],
                      0);
        }
      }
    }
...
#ifdef SHOW_PLAYER_DAMAGE
// don't bother testing for b here.
//             player.wstrength [0] = (pbullet[b].dam * 25) / (player.recycle1 /
//             player.recycle_speed1);
#endif
    } else {
    ...
    if (!key1 && key2 && !key3) {
      ...
      if (x != -900000) {
        ...
        quick_cloud(CLOUD_CIRCLE1, x, player.y - 9000, 0, -300, 30 + grand(10),
                    CLOUD_COL_1, 990);

        quick_cloud(CLOUD_CIRCLE2, x, player.y - 9000, -3000 + grand(500),
                    3000 + grand(500), 30 + grand(25), CLOUD_COL_1, 940);
        quick_cloud(CLOUD_CIRCLE2, x, player.y - 9000, -3000 + grand(500),
                    3000 + grand(500), 30 + grand(25), CLOUD_COL_1, 950);
        quick_cloud(CLOUD_CIRCLE2, x, player.y - 9000, -3000 + grand(500),
                    3000 + grand(500), 30 + grand(25), CLOUD_COL_1, 960);
        quick_cloud(CLOUD_CIRCLE2, x, player.y - 9000, 3000 - grand(500),
                    3000 + grand(500), 30 + grand(25), CLOUD_COL_1, 940);
        quick_cloud(CLOUD_CIRCLE2, x, player.y - 9000, 3000 - grand(500),
                    3000 + grand(500), 30 + grand(25), CLOUD_COL_1, 950);
    ...
    if ((key1 && key2) || key3) {
      ...
      if (player.recycle1 <= 0) {
        ...
        if (b != -1) {
          pbullet[b].x = player.x + xpart(player.ring_angle +
                                              (ANGLE_3 * player.orb_firing),
                                          player.ring_size << 10);
          pbullet[b].y = player.y + ypart(player.ring_angle +
                                              (ANGLE_3 * player.orb_firing),
                                          player.ring_size << 10);
          pbullet[b].x_speed = xpart(
              -ANGLE_4 +
                  xpart(player.ring_angle + (ANGLE_3 * player.orb_firing), 36),
              9000);
          pbullet[b].y_speed = ypart(
              -ANGLE_4 +
                  xpart(player.ring_angle + (ANGLE_3 * player.orb_firing), 36),
              9000);
          pbullet[b].xsize = 7000; // 9000 + player.wlevel [m] * 500;
        ...
        quick_cloud(CLOUD_CIRCLE2, pbullet[b].x, pbullet[b].y,
                    pbullet[b].x_speed, pbullet[b].y_speed,
                    20 + grand(10) + player.weapon_level, CLOUD_COL_1, 900);
#ifdef SHOW_PLAYER_DAMAGE
        // don't bother testing for b here.
        player.wstrength[0] =
            (pbullet[b].dam * 25) / (player.recycle1 / player.recycle_speed1);
  ...
  case PTYPE_HAND:
    ...
    if (key1 && !key2) {
      if (player.recycle1 <= 0) {
        ...
        player.recoil = 18;
        play_effectwfvx(WAV_SHOT2, 1350 - player.weapon_level * 10, 70,
                        player.x);
        for (f = 0; f < 2; f++) {
          ...
          if (b != -1) {
            ...
            quick_cloud(CLOUD_CIRCLE1, pbullet[b].x, pbullet[b].y,
                        pbullet[b].x_speed, pbullet[b].y_speed, 10 + grand(10),
                        CLOUD_COL_1, 0);
            quick_cloud(CLOUD_CIRCLE1, pbullet[b].x, pbullet[b].y,
                        pbullet[b].x_speed, pbullet[b].y_speed, 10 + grand(10),
                        CLOUD_COL_1, 600);
        ...
        //     quick_cloud(CLOUD_CIRCLE1, player.x - 8000, player.y - 22000, 0,
        //     -200, 10 + grand(10), CLOUD_COL_1, 990);
        //     quick_cloud(CLOUD_CIRCLE1, player.x + 8000, player.y - 22000, 0,
        //     -200, 10 + grand(10), CLOUD_COL_1, 990);

        /*     quick_cloud(CLOUD_CIRCLE4, player.x - 8000, player.y - 22000,
           -1000 + grand(2000), -1000 - grand(2000), 10 + grand(25),
           CLOUD_COL_1, 950); quick_cloud(CLOUD_CIRCLE4, player.x - 8000,
           player.y - 22000, -1000 + grand(2000), -1000 - grand(2000), 10 +
           grand(25), CLOUD_COL_1, 950); quick_cloud(CLOUD_CIRCLE4, player.x -
           8000, player.y - 22000, -1000 + grand(2000), -1000 - grand(2000), 10
           + grand(25), CLOUD_COL_1, 950); quick_cloud(CLOUD_CIRCLE4, player.x -
           8000, player.y - 22000, -1000 + grand(2000), -1000 - grand(2000), 10
           + grand(25), CLOUD_COL_1, 950); quick_cloud(CLOUD_CIRCLE4, player.x -
           8000, player.y - 22000, -1000 + grand(2000), -1000 - grand(2000), 10
           + grand(25), CLOUD_COL_1, 950); quick_cloud(CLOUD_CIRCLE4, player.x -
           8000, player.y - 22000, -1000 + grand(2000), -1000 - grand(2000), 10
           + grand(25), CLOUD_COL_1, 950);

             quick_cloud(CLOUD_CIRCLE4, player.x + 8000, player.y - 22000, -1000
           + grand(2000), -1000 - grand(2000), 10 + grand(25), CLOUD_COL_1,
           950); quick_cloud(CLOUD_CIRCLE4, player.x + 8000, player.y - 22000,
           -1000 + grand(2000), -1000 - grand(2000), 10 + grand(25),
           CLOUD_COL_1, 950); quick_cloud(CLOUD_CIRCLE4, player.x + 8000,
           player.y - 22000, -1000 + grand(2000), -1000 - grand(2000), 10 +
           grand(25), CLOUD_COL_1, 950); quick_cloud(CLOUD_CIRCLE4, player.x +
  ...
  case PTYPE_SQUID:
    ...
    if (key1) {
      if (player.orb_phase != ORB_DOCK) {
        if (player.recycle1 <= 0) {
          play_effectwfvx(WAV_SHOT2, 600 - player.weapon_level * 15, 90,
                          player.x);
          player.recycle1 = 1000;
          ...
          if (b != -1) {
            ...
            if (f == 1) {
              ...
              pbullet[b].x = player.x + 16000;
              pbullet[b].angle = 0;
              quick_cloud(CLOUD_CIRCLE1, pbullet[b].x, pbullet[b].y, 1000, 0,
                          7 + grand(5), CLOUD_COL_3, 900);
              quick_cloud(CLOUD_CIRCLE1, pbullet[b].x, pbullet[b].y, 3000, 0,
                          10 + grand(5), CLOUD_COL_3, 900);
              quick_cloud(CLOUD_CIRCLE1, pbullet[b].x, pbullet[b].y, 5000, 0,
                          15 + grand(5), CLOUD_COL_3, 900);
            } else {
              quick_cloud(CLOUD_CIRCLE1, pbullet[b].x, pbullet[b].y, -1000, 0,
                          7 + grand(5), CLOUD_COL_3, 900);
              quick_cloud(CLOUD_CIRCLE1, pbullet[b].x, pbullet[b].y, -3000, 0,
            ...
            pbullet[b].y_speed = 0;
            pbullet[b].xsize = 4000; // + player.weapon_level * 660;//9000 +
                                     // player.wlevel [m] * 500;
            pbullet[b].ysize = 4000; // + player.weapon_level * 660;//9000 +
                                     // player.wlevel [m] * 500;
            pbullet[b].level = player.weapon_level;
        ...
        if (player.recycle1 <= 0) {
          ...
          quick_cloud(CLOUD_CIRCLE4, pbullet[b].x, pbullet[b].y,
                      pbullet[b].x_speed, pbullet[b].y_speed,
                      20 + grand(10) + player.weapon_level, CLOUD_COL_3, 900);
        }
    }
    if (key2 || key3) {
...
void run_hand(void) {
  ...
  if (player.hand_phase != HAND_DOCK && size > 0) {
    for (i = 0; i < 3; i++) {
      //      quick_cloud(CLOUD_3FCIRCLE2, player.hand_x +
      //      xpart(player.hand_angle + ANGLE_3 * i, (((player.hand_unfold >> 1)
      //      + 2) << 10)), player.hand_y + ypart(player.hand_angle + ANGLE_3 *
      //      i, (((player.hand_unfold >> 1) + 2) << 10)), 0, 0, 10 + grand(5) +
      //      grand(player.weapon_level), CLOUD_COL_3, 0);
      quick_cloud(
          CLOUD_3FCIRCLE2,
          player.hand_x + xpart(player.hand_angle + ANGLE_3 * i,
                                (((player.hand_unfold >> 1) + 2) << 10)),
          player.hand_y + ypart(player.hand_angle + ANGLE_3 * i,
                                (((player.hand_unfold >> 1) + 2) << 10)),
          grand(500) - grand(500), grand(500) - grand(500),
          10 + grand(5) + grand(player.weapon_level), CLOUD_COL_3, 0);
    }
  } // remember to change also in draw_hand!

  if (player.hand_phase != HAND_DOCK && arena.counter & 1)
    quick_cloud(CLOUD_3FCIRCLE2, player.hand_x, player.hand_y,
                player.hand_x_speed + grand(2000) - grand(2000),
                player.hand_y_speed + grand(2000) - grand(2000),
                12 + grand(5) + (player.weapon_level), CLOUD_COL_1, 940);
  ...
  if (player.hand_unfold > 30 + player.weapon_level) {
    ...
    if (size > 0) {
      ...
      for (i = 0; i < 3; i++) {
        quick_cloud(
            CLOUD_3FCIRCLE2,
            player.hand_x + xpart(angle + ANGLE_3 * i,
                                  (((player.hand_unfold >> 2) + 3) << 10)),
            player.hand_y + ypart(angle + ANGLE_3 * i,
                                  (((player.hand_unfold >> 2) + 3) << 10)),
            0, 0, 7 + grand(3) + grand(player.weapon_level), CLOUD_COL_4, 0);
      }
      if (arena.counter % 8 == 0)
        play_effectwfvx(WAV_ROBOT, 1000 + player.hand_unfold * 8,
                        20 + player.hand_unfold, player.hand_x);
  ...
  */
  ...
  case HAND_DOCK:
    ...
    //       player.hand_angle += (player.hand_x_speed >> 8) +
    //       (player.hand_y_speed >> 8); player.hand_angle &= 1023;
    break;
  case HAND_SEEK:
    create_hand_bullet();
    if (lowest == 10) {
    ...
    angle =
        radians_to_angle(atan2(enemy[player.lock[lowest]].y - player.hand_y,
                               enemy[player.lock[lowest]].x - player.hand_x));
    ...
    player.hand_angle +=
        (player.hand_x_speed >> 8) + (player.hand_y_speed >> 8);
    player.hand_angle &= 1023;
    break;
  case HAND_RETURN:
    create_hand_bullet();
    angle = radians_to_angle(
        atan2((player.y + 12000) - player.hand_y, player.x - player.hand_x));
    player.hand_x_speed += xpart(angle, 400);
    player.hand_y_speed += ypart(angle, 400);
    drag_hand(960);
    player.hand_x += player.hand_x_speed;
    player.hand_y += player.hand_y_speed;
    if (player.hand_x > player.x - 9000 && player.hand_x < player.x + 9000 &&
        player.hand_y > player.y + 5000 && player.hand_y < player.y + 16000) {
  ...
  //     player.hand_angle += xpart(player.hand_x_speed >> 4, 12) +
  //     xpart((player.hand_y_speed + ANGLE_4) >> 4, 12); player.hand_angle +=
  //     (player.hand_x_speed >> 8) + (player.hand_y_speed >> 8);
...
void run_orb(void) {
  ...
  case ORB_RETURN:
    //       create_hand_bullet();
    angle = radians_to_angle(
        atan2((player.y - 18000) - player.orb_y, player.x - player.orb_x));
    ...
    if (player.orb_x > player.x - 12000 && player.orb_x < player.x + 12000 &&
        player.orb_y < player.y - 12000 && player.orb_y > player.y - 25000) {
...
void multi_fire(int m) {
  ...
  // see swbeam function in pbullet.c for damage
    ...
    case WPN_SEEKER:
     ...
     player.wfired = m;
  #ifdef SHOW_DAMAGE
     calc_damage(pbullet[0].dam * (3 + player.wlevel [m]), player.recycle_speed
  [m]);
  // see lwbeam function in pbullet.c for damage
  #endif
...
#ifdef SHOW_DAMAGE
void calc_damage(int dam, int speed) { damage_per_time = (dam * speed) / 100; }
#endif
...
void fire_seekers(int m, int num) {
  ...
  int angles[13] = {ANGLE_2,
                    ANGLE_2 + ANGLE_16,
                    ANGLE_2 - ANGLE_16,
                    ANGLE_2 + ANGLE_8,
                    ANGLE_2 - ANGLE_8,
                    ANGLE_2 + ANGLE_8 + ANGLE_16,
                    ANGLE_2 - ANGLE_8 - ANGLE_16,
                    ANGLE_4,
                    -ANGLE_4,
                    ANGLE_4 - ANGLE_8,
  ...
  for (i = 0; i < number; i++) {
    ...
    if (b != -1) {
      ...
      //   quick_cloud(CLOUD_CIRCLE3, pbullet[b].x, pbullet[b].y, 0, 0, 25,
      //   TCOL_ORANGE, 0);
