...
IMPORTANT TO REMEMBER:
 ...
 In the GIMP, must go to image|mode|indexed colour and change the settings
there.
...
void build_new_palette(int a, int b, int c, int d) {
  ...
  for (i = 1; i < 14; i++) {
    set_a_colour(i, current_base[i - 1][0], current_base[i - 1][1],
                 current_base[i - 1][2]);
  }

  for (i = 0; i < 13; i++) {
    set_a_colour(TRANS_RED1 + i, current_base[i][0] + 9, current_base[i][1],
                 current_base[i][2] - 3);
    set_a_colour(TRANS_RED2 + i, current_base[i][0] + 18,
                 current_base[i][1] + 1, current_base[i][2] - 7);
    set_a_colour(TRANS_RED3 + i, current_base[i][0] + 31,
                 current_base[i][1] + 5, current_base[i][2] / 2);
    set_a_colour(TRANS_RED4 + i, current_base[i][0] / 3 + 42,
                 current_base[i][1] / 3 + 25, current_base[i][2] / 4);
    set_a_colour(TRANS_RED5 + i, current_base[i][0] / 4 + 45,
                 current_base[i][1] / 4 + 32, current_base[i][2] / 5);

    set_a_colour(TRANS_GREEN1 + i, current_base[i][0], current_base[i][1] + 3,
                 current_base[i][2] - 3);
    set_a_colour(TRANS_GREEN2 + i, current_base[i][0] + 1,
                 current_base[i][1] + 10, current_base[i][2] - 5);
    set_a_colour(TRANS_GREEN3 + i, current_base[i][0] + 6,
                 current_base[i][1] + 20, current_base[i][2] / 2);
    //      set_a_colour(TRANS_GREEN3 + i, current_base [i] [0] + 3,
    //      current_base [i] [1] + 20, current_base [i] [2] / 2);
    set_a_colour(TRANS_GREEN4 + i, current_base[i][0] / 3 + 30,
                 current_base[i][1] / 3 + 30, current_base[i][2] / 4 + 10);
    set_a_colour(TRANS_GREEN5 + i, current_base[i][0] / 4 + 45,
                 current_base[i][1] / 4 + 39, current_base[i][2] / 5 + 15);
    //      set_a_colour(TRANS_GREEN4 + i, current_base [i] [0] / 2 + 23,
    //      current_base [i] [1] / 2 + 30, current_base [i] [2] / 4 + 5);
    //      set_a_colour(TRANS_GREEN5 + i, current_base [i] [0] / 3 + 37,
    //      current_base [i] [1] / 3 + 39, current_base [i] [2] / 5 + 12);

    set_a_colour(TRANS_BLUE1 + i, current_base[i][0] - 1,
                 current_base[i][1] - 1, current_base[i][2] + 7);
    set_a_colour(TRANS_BLUE2 + i, current_base[i][0] + 1,
                 current_base[i][1] + 1, current_base[i][2] + 15);
    set_a_colour(TRANS_BLUE3 + i, current_base[i][0] / 2 + 18,
                 current_base[i][1] / 2 + 18, current_base[i][2] / 2 + 30);
    set_a_colour(TRANS_BLUE4 + i, current_base[i][0] / 4 + 36,
                 current_base[i][1] / 4 + 36, current_base[i][2] / 4 + 50);
    set_a_colour(TRANS_BLUE5 + i, current_base[i][0] / 5 + 51,
                 current_base[i][1] / 5 + 51, current_base[i][2] / 5 + 65);

    set_a_colour(TRANS_PURPLE2 + i, current_base[i][0] + 15,
                 current_base[i][1] + 1, current_base[i][2] + 15);
    set_a_colour(TRANS_PURPLE3 + i, current_base[i][0] / 2 + 25,
                 current_base[i][1] / 2 + 12, current_base[i][2] / 2 + 25);
    set_a_colour(TRANS_PURPLE4 + i, current_base[i][0] / 4 + 42,
                 current_base[i][1] / 4 + 30, current_base[i][2] / 4 + 42);

    /*      set_a_colour(TRANS_BLUE1 + i, base_colours [i] [0], base_colours [i]
       [1], base_colours [i] [2] + 5); set_a_colour(TRANS_BLUE2 + i,
       base_colours [i] [0], base_colours [i] [1], base_colours [i] [2] + 10);
          set_a_colour(TRANS_BLUE3 + i, base_colours [i] [0], base_colours [i]
       [1], base_colours [i] [2] + 15); set_a_colour(TRANS_BLUE4 + i,
       base_colours [i] [0], base_colours [i] [1], base_colours [i] [2] + 20);
          set_a_colour(TRANS_BLUE5 + i, base_colours [i] [0], base_colours [i]
       [1], base_colours [i] [2] + 25);*/
  }
}
