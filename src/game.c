...
void run_blade(void) {
  ...
  case BLADE_RETURN_ACW_LEFT:
    ...
    if (player.blade_size <= 2000)
      player.blade_size = 2000;
    //         if (player.blade_angle <= ANGLE_2 - ANGLE_8 && player.blade_angle
    //         > ANGLE_4)
    if (player.blade_angle <= ANGLE_2 - ANGLE_8 &&
        player.blade_angle > ANGLE_4) {
      player.blade_pos = BLADE_LEFT;
    }
    player.blade_x = player.x + xpart(player.blade_angle, BLADE_DISTANCE);
    player.blade_y = player.y + ypart(player.blade_angle, BLADE_DISTANCE);
  ...
  case BLADE_RETURN_ACW_RIGHT:
    ...
    if (player.blade_size <= 2000)
      player.blade_size = 2000;
    if (player.blade_angle < ANGLE_16 ||
        player.blade_angle > ANGLE_1 - ANGLE_8) {
      player.blade_pos = BLADE_RIGHT;
    }
    player.blade_x = player.x + xpart(player.blade_angle, BLADE_DISTANCE);
    player.blade_y = player.y + ypart(player.blade_angle, BLADE_DISTANCE);
  ...
  case BLADE_CW_SLASH:
    ...
    if (player.blade_angle >= ANGLE_1 - ANGLE_8 ||
        (player.blade_angle > 0 && player.blade_angle < ANGLE_4)) {
  ...
  case BLADE_CW_SWING:
    ...
    player.blade_angle += blade_speed;
    player.blade_angle &= 1023;
    if (player.blade_angle >= ANGLE_2 - ANGLE_4 + 1 &&
        player.blade_angle < ANGLE_1 - ANGLE_4) {
      if (player.swing_hold) {
        player.blade_pos = BLADE_CW_SLASH;
        play_effectwfvx(WAV_SWEEP, 1000 - player.blade_redness * 20, 170,
                        player.x);
      } else
  ...
  //       player.swing_sprite_angle = radians_to_angle(atan2(player.wy [m] -
  //       player.y, player.wx [m] - player.x)); player.swing_sprite_angle /=
  ...
  //        player.old_blade_x [player.old_blade_pos] = player.blade_x -
  //        xpart(player.blade_angle, (player.blade_size >> 2));
  //        player.old_blade_y [player.old_blade_pos] = player.blade_y -
  //        ypart(player.blade_angle, (player.blade_size >> 2));

  //        xa = player.blade_size >> 1; //grand(player.blade_size / 2);

  player.old_blade_x[player.old_blade_pos] =
      player.blade_x - xpart(player.blade_angle, xa);
  player.old_blade_y[player.old_blade_pos] =
      player.blade_y - ypart(player.blade_angle, xa);
  ...
  // quick_cloud(CLOUD_3FCIRCLE4, player.blade_x - xpart(player.blade_angle,
  // xa), player.blade_y - ypart(player.blade_angle, xa), 0, 0, 10 + grand(20),
  // 0, 950);

  //  quick_cloud(CLOUD_3FCIRCLE4, player.blade_x - xpart(player.blade_angle,
  //  xa), player.blade_y - ypart(player.blade_angle, xa), old_x -
  //  player.blade_x, old_y - player.blade_y, 10 + grand(20), 0, 900);
  ...
  if (player.blade_redness > 10 - grand(4) &&
      player.blade_redness < 10 + grand(9) && grand(3) == 0)
    cloud_col = 3;

  xa = player.blade_size / 2;
  quick_cloud(CLOUD_3FCIRCLE2, player.blade_x + xpart(player.blade_angle, xa),
              player.blade_y + ypart(player.blade_angle, xa),
              player.blade_x - old_x, player.blade_y - old_y,
              10 + grand(10 + player.weapon_level), cloud_col, 820);

  xa = grand(player.blade_size);
  quick_cloud(CLOUD_3FCIRCLE4, player.blade_x + xpart(player.blade_angle, xa),
              player.blade_y + ypart(player.blade_angle, xa),
              player.blade_x - old_x + grand(2000) - grand(2000),
              player.blade_y - old_y + grand(2000) - grand(2000),
              10 + grand(20 + player.weapon_level), cloud_col, 820);
  xa = grand(player.blade_size);
  quick_cloud(CLOUD_3FCIRCLE4, player.blade_x + xpart(player.blade_angle, xa),
              player.blade_y + ypart(player.blade_angle, xa),
              player.blade_x - old_x + grand(2000) - grand(2000),
              player.blade_y - old_y + grand(2000) - grand(2000),
              10 + grand(20 + player.weapon_level), cloud_col, 720);
  xa = grand(player.blade_size);
  quick_cloud(CLOUD_3FCIRCLE4, player.blade_x + xpart(player.blade_angle, xa),
              player.blade_y + ypart(player.blade_angle, xa),
              player.blade_x - old_x + grand(2000) - grand(2000),
              player.blade_y - old_y + grand(2000) - grand(2000),
              10 + grand(20 + player.weapon_level), cloud_col, 620);

  /*  xa = player.blade_size / 2;
    quick_cloud(CLOUD_3FCIRCLE2, player.blade_x - xpart(player.blade_angle, xa),
    player.blade_y - ypart(player.blade_angle, xa), player.blade_x - old_x,
    player.blade_y - old_y, 10 + grand(20), cloud_col, 820);

    xa = grand(player.blade_size);
    quick_cloud(CLOUD_3FCIRCLE4, player.blade_x - xpart(player.blade_angle, xa),
    player.blade_y - ypart(player.blade_angle, xa), player.blade_x - old_x +
    grand(2000) - grand(2000), player.blade_y - old_y + grand(2000) -
    grand(2000), 10 + grand(20), cloud_col, 820); xa = grand(player.blade_size);
    quick_cloud(CLOUD_3FCIRCLE4, player.blade_x - xpart(player.blade_angle, xa),
    player.blade_y - ypart(player.blade_angle, xa), player.blade_x - old_x +
    grand(2000) - grand(2000), player.blade_y - old_y + grand(2000) -
    grand(2000), 10 + grand(20), cloud_col, 720); xa = grand(player.blade_size);
    quick_cloud(CLOUD_3FCIRCLE4, player.blade_x - xpart(player.blade_angle, xa),
    player.blade_y - ypart(player.blade_angle, xa), player.blade_x - old_x +
    grand(2000) - grand(2000), player.blade_y - old_y + grand(2000) -
    grand(2000), 10 + grand(20), cloud_col, 620);
  */
  /* int c = quick_cloud(CLOUD_BLADE_TRAIL, player.blade_x, player.blade_y, 0,
   0, 15, 0, 0); if (c != -1)
   {
    cloud[c].x2 = old_x;
    cloud[c].y2 = old_y;
  ...
  if (b != -1) {
    pbullet[b].x =
        player.blade_x + xpart(player.blade_angle,
                               player.blade_size >> 1); // [player.swing_pos];
    pbullet[b].y =
        player.blade_y + ypart(player.blade_angle,
                               player.blade_size >> 1); // [player.swing_pos];
    ...
    //  pbullet[b].dam = ((300 - player.blade_redness * 9) * (10 +
    //  player.weapon_level)) / 10;// + player.wlevel [player.blade_index] * 19;
    //  // was 100/22 or 60/14
    pbullet[b].dam =
        ((175 - player.blade_redness * 5) * (10 + player.weapon_level)) /
        10; // + player.wlevel [player.blade_index] * 19; // was 100/22 or 60/14
  ...
  /* if (player.wlevel [player.blade_index] > 6)
   {
    int bangle = radians_to_angle(atan2(old_y - player.swing_y
   [player.swing_pos], old_x - player.swing_x [player.swing_pos])); bangle -=
   ANGLE_4; quick_cloud(CLOUD_CIRCLE2, old_x + xpart(bangle, 11000), old_y +
   ypart(bangle, 11000), 0, 0, player.wlevel [player.blade_index] - 2,
   TCOL_ORANGE, 0); bangle += ANGLE_2; quick_cloud(CLOUD_CIRCLE2, old_x +
   xpart(bangle, 11000), old_y + ypart(bangle, 11000), 0, 0, player.wlevel
   [player.blade_index] - 2, TCOL_ORANGE, 0);
   }*/
}
...
void player_hit(void) {
  ...
  for (i = 0; i < 5; i++) {
    angle = grand(ANGLE_1);
    xa = 20000 + grand(4400);
    quick_cloud(CLOUD_CIRCLE4, x + xpart(angle, xa), y + ypart(angle, xa),
                xpart(angle, xa / 15), ypart(angle, xa / 15), 40 + grand(26),
                CLOUD_COL_1, 960);
  }
  ...
  for (i = 0; i < 10; i++) {
    angle = grand(ANGLE_1);
    xa = 2000 + grand(5400);
    quick_cloud(CLOUD_FCIRCLE2, x, y, xpart(angle, xa), ypart(angle, xa),
                30 + grand(10), CLOUD_COL_1, 980);
    quick_cloud(CLOUD_FCIRCLE4, x, y, xpart(angle, xa - 1000),
                ypart(angle, xa - 1000), 30 + grand(10), CLOUD_COL_1, 980);
    quick_cloud(CLOUD_FCIRCLE4, x, y, xpart(angle, xa - 2000),
                ypart(angle, xa - 2000), 25 + grand(10), CLOUD_COL_1, 980);

    xa = 2000 + grand(10000);
    quick_cloud(CLOUD_CIRCLE4, x + xpart(angle, xa), y + ypart(angle, xa), 0, 0,
...
void init_player(void) {
  ...
  if (arena.level == 2) {
    player.wlevel[0] =
        options.stage_power[player.type - 1][0][arena.difficulty];
    player.wlevel[1] =
        options.stage_power[player.type - 1][0][arena.difficulty];
    player.wlevel[2] =
        options.stage_power[player.type - 1][0][arena.difficulty];
  ...
  if (arena.level == 3) {
    player.wlevel[0] =
        options.stage_power[player.type - 1][1][arena.difficulty];
    player.wlevel[1] =
        options.stage_power[player.type - 1][1][arena.difficulty];
    player.wlevel[2] =
        options.stage_power[player.type - 1][1][arena.difficulty];
...
void run_blade(void)
{
       ...
       switch(player.blade_pos)
       {
        ...
        case BLADE_ACW_SLASH:
         ...
         player.swing_angle -= BLADE_SPEED;
         player.swing_angle &= 1023;
         if (player.swing_angle <= ANGLE_2 - ANGLE_16 + 48 && player.swing_angle
> ANGLE_4)
         {
            if (player.swing_hold == 1)
            {
...
< ANGLE_1 - ANGLE_4)
       ...
       player.swing_sprite_angle = radians_to_angle(atan2(player.wy [m] -
player.y, player.wx [m] - player.x)); player.swing_sprite_angle /= (ANGLE_1 /
SMALL_ROTATIONS);
...
xpart(player.swing_angle, 12000); player.swing_y [player.swing_pos] = player.wy
[m] + ypart(player.swing_angle, 12000); player.swing_x2 [player.swing_pos] =
player.wx [m] + xpart(player.swing_angle, 25000); player.swing_y2
[player.swing_pos] = player.wy [m] + ypart(player.swing_angle, 25000);

        player.swing_strength [player.swing_pos] = player.wstrength
[player.blade_index];
 ...
 if (b != -1)
 {
  ...
  pbullet[b].dam = 80 + player.wlevel [player.blade_index] * 19; // was 100/22
or 60/14 pbullet[b].dam /= player.wstrength [player.blade_index];
  pbullet[b].source = player.blade_index;
 }
 if (player.wlevel [player.blade_index] > 6)
 {
  int bangle = radians_to_angle(atan2(old_y - player.swing_y [player.swing_pos],
old_x - player.swing_x [player.swing_pos])); bangle -= ANGLE_4;
  quick_cloud(CLOUD_CIRCLE2, old_x + xpart(bangle, 11000), old_y + ypart(bangle,
11000), 0, 0, player.wlevel [player.blade_index] - 2, TCOL_ORANGE, 0); bangle +=
ANGLE_2; quick_cloud(CLOUD_CIRCLE2, old_x + xpart(bangle, 11000), old_y +
ypart(bangle, 11000), 0, 0, player.wlevel [player.blade_index] - 2, TCOL_ORANGE,
0);
 }
...
#define MAX_PANELS 30
    ...
    for (j = 0; j < number; j++) {
      panel_y[0][j] += speed;
      if (panel_x_size[0][j] == 0 ||
          panel_y[0][j] > 616400 + (panel_y_size[0][j] << 10))
        new_panel(0, j);
    }
    ...
    for (j = 0; j < number; j++) {
      panel_y[1][j] += speed;
      if (panel_x_size[1][j] == 0 ||
          panel_y[1][j] > 616400 + (panel_y_size[1][j] << 10))
        new_panel(1, j);
    }
    ...
    for (j = 0; j < number; j++) {
      panel_y[2][j] += speed;
      if (panel_x_size[2][j] == 0 ||
          panel_y[2][j] > 616400 + (panel_y_size[2][j] << 10))
        new_panel(2, j);
    }
...
void new_panel(int i, int j) {
  ...
  case 0:
    ...
    if (grand(4) == 0)
      panel_x[i][j] += grand(50000) - grand(50000);
  ...
  case 1:
    ...
    if (grand(4) == 0)
      panel_x[i][j] += grand(100000) - grand(100000);
  ...
  case 2:
    ...
    if (grand(4) == 0)
      panel_x[i][j] += grand(130000) - grand(130000);
