...
enum { SOUNDMODE_STEREO, SOUNDMODE_MONO, SOUNDMODE_REVERSED };
...
// when on the next tick we're going to play a note with an attached event, set
// this to something
...
enum { SM_BEAT, SM_TONE, SM_LENGTH, SM_VOL, SM_PAN, SM_ALT_TONE, SM_SIZE };
...
void init_sound(void) {
  ...
  if (install_sound(DIGI_AUTODETECT, MIDI_AUTODETECT, NULL) == -1) {
    exit(1); // don't bother with proper error handling yet. This should just
             // disable sound.
  }
  set_volume(255, 0);
...
void run_beat(void) {
  ...
  for (i = 0; i < SUBMUSICS; i++) {
    if (submusic_playing[i] == SUBMUSIC_EMPTY)
      continue; // remember to set finished to 1 when setting submusic to
                // _empty!!!
    submusic_count[i]--;
    ...
    if (submusic_count[i] <= 0) {
      if (submusic[submusic_playing[i]][submusic_pos[i]][SM_BEAT] == BEAT_END) {
        ...
        //      submusic_pos [i] = 0;
        // we've finished the submusic, including any pause at the end, so set a
        // new submusic here
        continue;
      }
      ...
      /*    play_beat(submusic [submusic_playing [i]] [submusic_pos [i]]
         [SM_BEAT], tone [submusic [submusic_playing [i]] [submusic_pos [i]]
         [SM_TONE]], submusic [submusic_playing [i]] [submusic_pos [i]]
         [SM_VOL], submusic [submusic_playing [i]] [submusic_pos [i]]
         [SM_PAN]);*/
      submusic_count[i] =
          submusic[submusic_playing[i]][submusic_pos[i]][SM_LENGTH];
      //    if (arena.level == 3)
      //     submusic_count [i] = submusic [submusic_playing [i]] [submusic_pos
      //     [i]] [SM_LENGTH] * 0.8;
      submusic_pos[i]++;
    }
    //   if (submusic_pos [i]
...
void change_music(int mto) { to_change_music = mto; }
...
*/
void play_effect(int sample) { play_sample(sounds[sample], 200, 127, 1000, 0); }

/*
plays sample at frequency f (normal is 1000), volume v (255 is max), x position
x (middle is 127)
*/
...
void play_effectwfvx(int sample, int f, int v, int x) {
  // return;
  //  int pan = x / 2500; // this gives us a # from 0 to 255, from the left of
  //  the screen to the right. For stereo

  int pan = x / 5030; // this gives us a # from 0 to 255, from the left of the
                      // screen to the right. For stereo
  pan += 64;
