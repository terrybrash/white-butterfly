...
This file contains some maths and utility functions. The only ones used at
present are:

xpart/ypart - call them with angle x and distance d; they'll give you the x
component and y component of a line in the angle x and the distance d

angle_to_radians - I use a 1024-degree circle in a format called 'angle'. It
lets me use lookup tables radians_to_angle   without interpolation for simple
trig, and minimises slow floating-point operations. angle_to_radians takes
'angle' values and turns them into radians; radians_to_angle etc.
...
// I have no idea why, but the first few elements of cos_table always get
// corrupted
//  unless I put a big fat decoy array just above. A similar thing happens to
//  the palette arrays; allegro seems to have a problem with global arrays like
//  these.
...
float lcos(int angle) { return cos_table[angle & 1023]; }

float lsin(int angle) { return sin_table[angle & 1023]; }

float angle_to_radians(int angle) { return ((float)angle * PI * 2) / ANGLE_1; }
...
fixed angle_to_fixed(int angle) { return itofix(angle / ANGLE_TO_FIXED); }
