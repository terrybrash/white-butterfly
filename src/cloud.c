...
int quick_cloud(int type, int x, int y, int x_speed, int y_speed, int timeout,
                int colour, int drag);
...
void run_clouds(void) {
  ...
  for (c = 0; c < NO_CLOUDS; c++) {
    ...
    case CLOUD_RUMBLE:
      ...
      quick_cloud(CLOUD_FCIRCLE1, cloud[c].x + xpart(angle, ya),
                  cloud[c].y + ypart(angle, ya), 0, 0, xa + grand(xa),
                  cloud[c].colour, 0);
      break;
...
void destroy_cloud(int c) { cloud[c].type = CLOUD_NONE; }
