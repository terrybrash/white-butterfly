...
/*
Is called every 25ms; lets the game know that it's time to move on to the next
frame.
*/
void tickover(void) { ticked++; }
...
int main(void) {
  ...
  if (allint == -1) {
    ...
    allegro_message(
        "Failed to initialise Allegro! This isn't going to work, sorry.");
    exit(1);
  }
...
void init_at_startup(void) {
  ...
  /*
  The following code is for use if your system is set up to prevent an
  application writing to its own directory. Normally this will prevent
  highscores and key configurations being saved, but the following code puts the
  initfile in a better place. This may be a default on some systems (eg Unixes
  and possibly Windows Vista) and can be set on some other systems.

  The only problem with this is that you'll have to manually type or copy the
  various other options (windowed, vsync, joystick buttons etc) into the
  initfile in this new location (or just copy the initfile across, or just rely
  on the default values and let the game make a new initfile).

  Thanks to Thomas Harte for this code! I had no idea it was necessary (it isn't
  on my Windows XP computer, and I haven't tested it).
...
#define UNIX_OSX_VISTA_ETC
  ...
  */
  ...
  for (i = 0; i < 3; i++) {
    ...
    options.stage_power_bar[0][0][i] =
        get_config_int(diff_string, "power_bar0_0", 0);
    options.stage_power_bar[1][0][i] =
        get_config_int(diff_string, "power_bar1_0", 0);
    options.stage_power_bar[2][0][i] =
        get_config_int(diff_string, "power_bar2_0", 0);
    options.stage_power_bar[3][0][i] =
        get_config_int(diff_string, "power_bar3_0", 0);
    options.stage_power_bar[4][0][i] =
        get_config_int(diff_string, "power_bar4_0", 0);

    options.stage_power_bar[0][1][i] =
        get_config_int(diff_string, "power_bar0_1", 0);
    options.stage_power_bar[1][1][i] =
        get_config_int(diff_string, "power_bar1_1", 0);
    options.stage_power_bar[2][1][i] =
        get_config_int(diff_string, "power_bar2_1", 0);
    options.stage_power_bar[3][1][i] =
        get_config_int(diff_string, "power_bar3_1", 0);
    options.stage_power_bar[4][1][i] =
        get_config_int(diff_string, "power_bar4_1", 0);
