...
enum { HAND_DOCK, HAND_SEEK, HAND_STOP, HAND_RETURN };
...
struct playerstruct {
  ...
  int wstrength[3]; // stores damage per second information for display -
                    // debugging/balancing
...
enum { MULTI_NONE, MULTI_BASIC };

enum { RANK_C, RANK_B, RANK_A, RANK_S, RANK_P };
...
struct arenastruct {
  ...
  /*
    ...
    int part_x1 [18] [16]; // just needs to be [18] [14] but I added padding
    after a stupid mistake. int part_y1 [18] [16]; int part_x2 [18] [16]; int
    part_y2 [18] [16]; int part_cx [18] [16]; int part_cy [18] [16]; int
    part_flow; int part_rank; int wave_y; int part_pulse1; int part_pulse2;
...
struct optionstruct {
  int sound_init; // if 0, sound isn't initialised at all. Changed in config
                  // file only.
